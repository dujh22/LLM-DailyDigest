...
source = "机器之心"
summary = "ScienceDiscovery v0.1 是基于 Agent OS 构建的可溯源一站式 AI 科研工作台，通过多智能体、300 余项科研 Skills、结构化记忆与审核机制覆盖全流程科研，并以树搜索、沙箱评分和异步扩展实现无需训练模型的科研代码递归改进。"
paper = ""
code = "https://github.com/openJiuwen-ai/sciencediscovery"
dataset = ""
link = "https://mp.weixin.qq.com/s/XSaX6oyok9qqZmwBUiQCFg"
content = "ScienceDiscovery 基于 Agent OS（JiuwenSwarm）构建，借助 MCP Connector、多智能体动态编排和 300 余项跨领域 Skills，覆盖文献阅读、假设提出、代码编写、实验试错与参数调优，并在隔离沙箱中运行 Python、R 和 Shell。平台以结构化记忆图谱记录任务时序、代码、环境、日志和结论证据链，结合多级 Reviewer 审核提升结果的可追溯性与可复现性；基于 GLM-5.2，其在 BiomniBench-DA 上获得 77.4 分，并已用于脓毒症分析、纳米抗体设计和跨库文献调研。其产物级 RSI 将科研代码迭代表示为搜索树，通过父版本选择、模型改写、沙箱执行评分、访问衰减与合并治理，在不更新模型权重或搜索规则的情况下平衡高分分支深挖和其他路径探索，并支持多工作器异步扩展。实验中，系统在振荡积分任务两小时生成 236 个版本并产出通用积分器，在 AlgoTune 上实现平均 2.279 倍加速，在 LLM-SRBench 子集的 111 道题中正确恢复 41.4% 的方程。该演进框架可替换树搜索、遗传变异、爬山和存档等策略，但向湿实验等领域扩展仍依赖快速、低成本且可信的机器验证，以及仿真、代理模型或自动化实验设施。"
purpose = "- 为跨学科团队提供从资料检索、数据分析、代码执行到报告生成的一体化科研环境，减少工具切换与人工试错成本。\n- 以任务图谱、原始代码、运行环境、日志和审核记录构建证据链，降低虚假引用、错误推导及实验不可复现风险。\n- 展示 MCP 工具标准化、多智能体分工、领域 Skills 与产物级搜索相结合的自动科研工程路径，无需微调模型即可持续优化科研代码。\n- 可用于生物医药设计、科学数据分析、数值算法生成、代码加速、符号回归及跨库文献证据综合。\n- 启示科研 RSI 的关键瓶颈可能从候选生成转向验证速度和评分可信度，应优先建设安全沙箱、可靠验证器，并结合仿真、代理模型和自动化实验设施拓展应用。"
notes = "Auto Research|ScienceDiscovery发布：零科研幻觉的一站式AI工作台，BiomniBench-DA验证效果业界SOTA https://mp.weixin.qq.com/s/XSaX6oyok9qqZmwBUiQCFg\n\n[合并自 2026-09-07 sciencediscovery-rsi]\n标题：ScienceDiscovery实现树搜索驱动RSI，加速科学发现，小时级写出通用积分器，低成本找出物理科学规律\n来源：量子位（微信公众号）\n链接：http://mp.weixin.qq.com/s?__biz=MzIzNjc1NzUzMw==&mid=2247918839&idx=2&sn=c888eb2ab4fcd88b5d1c8c8b178f4c73&chksm=e9403cfe237c3c6b3eb6890fd0765a93e91b7f0a72a63c5609b25296676045a8616c4fe5fff4&scene=0&xtrack=1#rd\n摘要：\n\n[合并自 2026-09-07 sciencediscovery]\n标题：九问ScienceDiscovery实现树搜索驱动RSI，加速科学发现，小时级写出通用积分器，低成本找出物理科学规律\n来源：量子位（微信公众号）\n链接：https://www.qbitai.com/2026/09/484293.html\n摘要：不训练模型、不调参数"
