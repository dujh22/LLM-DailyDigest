...
source = "arXiv"
summary = "SPADE 让同一 LLM 分别扮演环境设计者与推理智能体，通过生成可执行的长程环境并围绕能力边界自博弈，实现开放式持续改进。"
paper = "https://arxiv.org/abs/2608.19197"
code = ""
dataset = ""
link = ""
content = "SPADE 将环境设计纳入学习过程，由环境设计者生成带有 `reset()`/`step()` 接口、状态转移、奖励函数和验证代码的完整可执行环境，推理智能体则在其中学习行动。框架以有无特权提示时的奖励差估计智能体遗憾，并据此让设计者生成位于智能体能力边缘且仍可完成的任务。实验表明，利用预训练语料文档约束环境生成并积累环境记忆，是提升训练效果的关键。扩展至 30B 参数模型后，SPADE 在八项数学、科学、代码和推理基准上平均领先最强固定环境基线 5.3 分，并显著改善多轮工具调用与智能体任务表现。"
purpose = "- 将训练环境本身设为可学习对象，为语言智能体持续生成难度自适应的新目标。\n- 统一推理题、工具调用和游戏等任务的可执行环境接口，便于构建通用自博弈训练系统。\n- 通过遗憾信号定位能力边界，可用于动态课程学习并减少任务过易或不可解的问题。\n- 环境记忆与文档 grounding 的有效性说明，开放式自我改进既需要生成多样性，也需要知识约束和历史经验复用。"
...
dataset = ""
link = ""
content = "研究将 AI 后训练能力区分为执行层能力与策略层能力：前者是在既定训练方案内迭代，后者是根据实验结果修正高层判断。对大量公开后训练轨迹的分析表明，智能体通常在流程开始时便锁定策略，随后预算几乎全部用于局部参数与实现调整。经验驱动脚手架可显著提升执行表现，在 GSM8K 和 HumanEval 上分别提高 12.6 和 40.8 分，但未能促使策略变化；人工指导能改变初始方向，却无法阻止智能体重新陷入局部调整循环。增加推理算力只对较简单任务有效，最困难任务几乎没有收益，说明核心缺口是执行期间自发重估策略的机制。"
purpose = "- 提醒自主后训练系统不能只优化代码执行、实验启动和检查点评估，还需显式支持高层策略更新。\n- 可将“是否依据新证据改变训练路线”纳入 AI-for-AI 与自我改进系统的核心评测指标。\n- 为后训练智能体设计策略级反馈、变点检测、备选假设维护和主动回滚机制提供依据。\n- 表明增加经验库、人工提示或推理预算并不必然解决策略固化问题，应优先研究闭环元决策机制。"
...
source = "arXiv"
summary = "论文提出 Harness Continual Learning，让冻结基础模型通过持续演化提示、记忆、工具、技能与路由规则来积累能力，同时评估并抑制脚手架层面的遗忘。"
paper = "https://arxiv.org/abs/2608.19013"
code = ""
dataset = ""
link = ""
content = "论文将持续学习从模型参数扩展到模型外部的执行脚手架，并定义了更新脚手架后既有可靠行为退化的“脚手架级遗忘”。HCL 由任务接口、经验记忆、能力地图和自适应路由器四类面向执行的组件构成，基础模型保持冻结。其受控脚手架演化机制将候选更新生成与状态提交分离，仅在持续评估器确认当前能力提升、历史能力保留且更新有效后才提交。文本推理、多模态感知和开放世界交互实验显示，该方法能够积累能力并从失败中恢复，在多个场景中相对基线提升超过 10%，且可显式调节稳定性与可塑性的权衡。"
purpose = "- 为冻结模型提供一种无需修改参数的持续适应路径，把提示、记忆、工具和路由纳入统一的可演化状态。\n- 提醒智能体系统不能只关注模型层遗忘，还需测试脚手架更新对历史任务与可靠行为的破坏。\n- 可借鉴“生成候选更新—回归评估—验证后提交”的机制，为生产智能体建立安全的配置演化与状态回滚流程。\n- 为 HarnessEvolve 研究提供了组件化架构、遗忘度量以及稳定性—可塑性调节范式。"
...
source = "arXiv"
summary = "FACET以修复后的容器状态为共享接地，统一生成指令、参考解法与验证器，从而提升合成终端任务的有效性、一致性及训练价值。"
paper = "https://arxiv.org/abs/2608.18580"
code = ""
dataset = ""
link = ""
content = "FACET面向终端智能体训练中任务不可解、验证错误及源信息丢失等问题，强调保留原始技能中的目标、依赖、状态迁移与流程约束。框架先将相关技能重构为信息丰富的连贯场景，再实现并修复执行环境，最后基于同一容器状态生成任务指令、参考解法和可执行验证器。它通过执行验证和定向修复处理特定工件的故障，避免重复生成已经有效的组件。由此合成的任务具有更复杂的操作流程和更密集的可执行检查，其成功轨迹用于微调不同规模模型后，均提升了 Terminal-Bench 2.1 表现。"
purpose = "- 为大规模生成可执行的终端智能体训练数据提供环境优先的构造范式。\n- 说明指令、解法与验证器应共享真实执行状态，以减少跨工件假设不一致导致的错误监督。\n- 可将执行验证与局部修复引入数据生产流水线，在保留有效组件的同时降低重新生成成本。\n- 表明保留来源意图和程序约束，有助于合成更复杂、有效且数据效率更高的智能体任务。"
