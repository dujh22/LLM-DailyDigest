...
source = "arXiv"
summary = "Eureka 将长程科研任务编译为带明确验收语义的动态义务图，并通过成本收益门控按需组建、验证和演化具备专用记忆、工具及验证机制的宏代理架构。"
