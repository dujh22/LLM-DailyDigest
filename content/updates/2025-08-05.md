...
[[items]]
id = "goedel-prover-v2"
title = "Goedel-Prover-V2：通过分阶段数据合成与自我纠正扩展形式定理证明"
subtopic = "定理证明"
topics = [
    "推理",
    "逻辑推理",
...
research = []
source = "轻舟AI（小红书）"
summary = "清华大学最新研究探索借鉴人类分层思考方式，以改进大模型的推理过程。"
paper = ""
code = ""
dataset = ""
link = "https://www.xiaohongshu.com/discovery/item/68917d470000000004006bd4?source=webshare&xhsshare=pc_web&xsec_token=CBNjhAAQ9ahkh4hZ_0xmlIBrpbqQrNiMfkjm4OBI-IQ04=&xsec_source=pc_share"
content = "该内容介绍了清华大学一项与分层推理相关的最新研究。其核心思路是借鉴人类由高层思考到具体推演的认知方式，组织模型的推理过程。原始文本未提供论文名称、具体方法、实验结果或开源信息，详情需参阅原帖。"
purpose = "- 为大模型复杂任务推理提供分层组织思路\n- 启发将抽象规划与具体推演相结合的研究设计\n- 可用于关注长链推理、逻辑推理与认知启发式方法的进展"
notes = "1. 80 【借鉴人类思考的分层推理？清华大学最新研究 - 轻舟AI | 小红书 - 你的生活兴趣社区】 😆 M49Up0rsYbIO6Xd 😆 https://www.xiaohongshu.com/discovery/item/68917d470000000004006bd4?source=webshare&xhsshare=pc_web&xsec_token=CBNjhAAQ9ahkh4hZ_0xmlIBrpbqQrNiMfkjm4OBI-IQ04=&xsec_source=pc_share"


[[items]]
id = "frontier"
title = "Frontier：模拟下一代大语言模型推理系统"
subtopic = "推理系统"
topics = [
    "推理",
    "系统优化",
...
research = []
source = "arXiv"
summary = "SynAdapt 通过合成连续思维链对齐目标与难度分类器，在保持推理准确率的同时降低离散思维链带来的生成成本。"
paper = ""
code = ""
dataset = ""
link = ""
content = "SynAdapt 针对离散思维链生成耗时高、现有连续思维链方法对齐不足和目标不一致等问题，引入合成连续思维链作为精确的训练目标。该目标直接指导大语言模型学习连续推理表示并输出答案，从而改善推理效率。框架还结合问题上下文与连续思维链训练难度分类器，在短暂推理后识别较难的问题。对于难题，系统会自适应地提示模型重新思考，实验表明其在多种难度基准上取得了较优的准确率与效率权衡。"
purpose = "- 利用合成连续思维链构造更直接、精确的推理对齐数据。\n- 减少离散思维链逐 token 生成造成的推理延迟与计算开销。\n- 通过难度分类与自适应重思机制，将额外计算集中用于困难样本。\n- 为按题目难度动态分配推理预算提供可复用的框架思路。"
...
[[items]]
id = "corgi"
title = "CoRGI：以视觉证据验证思维链的多模态推理框架"
subtopic = "多模态推理"
topics = [
    "数据合成",
    "多模态",
...
research = []
source = "arXiv"
summary = "CoRGI通过提取视觉证据并与文本思维链合成，在无需端到端重训的情况下提升视觉语言模型推理的真实性与稳健性。"
paper = ""
code = ""
dataset = ""
link = ""
content = "CoRGI 是一个面向视觉语言模型的模块化推理框架，旨在减少思维链解释缺乏视觉依据的问题。其流程先生成文本推理链，再由 VEVM 模块为每个推理步骤提取支持性视觉证据，最后将文本依据与视觉证据合成为经过验证的答案。该框架无需端到端重新训练，可直接集成到现有 VLM 中。在 VCR 基准上，CoRGI 提升了 Qwen-2.5VL 和 LLaVA-1.6 的推理表现，人类评估也显示其解释更加真实、有用。"
purpose = "- 为多模态思维链增加逐步视觉验证机制，降低语言流畅但脱离图像内容的幻觉。\n- 展示了一种将文本推理与视觉证据进行模块化合成的数据构造思路。\n- 可作为现有视觉语言模型的事后验证层，无需高成本的端到端重新训练。\n- 提示多模态推理系统应显式检查中间步骤，而不应只验证最终答案。"
...
[[items]]
id = "claude-opus-4-1-swe-bench-74-5"
title = "Claude Opus 4.1 发布，SWE-bench 成绩提升至 74.5%"
subtopic = "代码模型"
topics = [
    "新模型",
    "AI编程",
...
research = []
source = "微信公众号"
summary = "Genie 3 可根据文本生成支持实时交互的动态 3D 世界，并以 720P、24 FPS 运行及保持分钟级环境一致性。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/ulhJGiiq301f1yuPRTl36g"
content = "谷歌 DeepMind 发布第三代通用世界模型 Genie 3，用户可通过文本提示生成并实时探索动态 3D 环境。相比上一代，Genie 3 支持 720P 画质和每秒 24 帧的实时导航。模型能够在分钟级交互过程中维持环境与物体状态的一致性，生成结果也更符合物理规律。该模型延续了 DeepMind 在模拟环境、开放式学习、游戏智能体和机器人训练等方向上的长期研究。"
purpose = "- 为智能体提供可实时交互的训练与评估环境，降低构建专用模拟器的成本\n- 推动开放世界探索、机器人学习和具身智能研究\n- 分钟级环境一致性表明世界模型正从短视频生成走向可持续交互的环境模拟"
notes = "2. **谷歌DeepMind发布了****新一代通用世界模型Genie 3**\n   1. [谷歌“世界模拟器”深夜上线！一句话生成3D世界，支持分钟级超长记忆](https://mp.weixin.qq.com/s/ulhJGiiq301f1yuPRTl36g)\n      1. **Genie 3相比上一代大幅升级，支持****720P画质，每秒24帧实时导航，以及分钟级的一致性保持**。\n      2. **最让谷歌引以为傲的，还要属Genie 3的****长期环境一致性**。\n      3. **DeepMind十多年来一直在关注模拟环境领域的研究，从训练智能体掌握实时战略游戏， 到开发用于开放式学习和机器人技术的模拟环境。**\n   2. [谷歌推出「G」字号第三代世界模型Genie 3，号称「宇宙模拟器」，视频生成更加符合物理定律。](https://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652617195&idx=1&sn=91f7f14b4c811e2a1cd9bb5cf5652a11&scene=21#wechat_redirect)"


[[items]]
id = "openai-gpt-oss-120b-gpt-oss-20b"
title = "OpenAI 开源推理模型 gpt-oss-120b 与 gpt-oss-20b"
subtopic = "开源模型"
topics = [
    "新模型",
    "推理",
...
research = []
source = "arXiv"
summary = "文章综述大语言模型在非形式数学与形式定理证明中的最新进展，并分析证明生成较代码合成更脆弱及模型能否追踪逻辑状态等核心问题。"
paper = ""
code = ""
dataset = ""
link = ""
content = "大型语言模型在结构化推理、符号任务和代码生成上表现突出，但其在形式数学与定理证明方面的进展相对缓慢。文章结合近期模型与基准，讨论非形式数学和形式数学作为训练领域时各自的优势与权衡。作者进一步分析证明生成为何比代码合成更脆弱，以及现有监督方式能否促使模型稳定追踪演绎过程。文章还探讨模型究竟形成了持续演化的逻辑状态表征，还是仅在表面上模仿推理轨迹。"
purpose = "- 帮助梳理大语言模型数学推理与形式定理证明的技术边界\n- 为形式数学和非形式数学训练数据的配比与选择提供参考\n- 提醒研究者关注证明状态追踪、过程监督和推理可靠性\n- 为设计更有效的数学推理基准与评估方法提供研究方向"
...
[[items]]
id = "board-game-arena"
title = "Board Game Arena：通过策略游戏评估大型语言模型的框架与基准"
subtopic = "博弈评测"
topics = [
    "逻辑推理",
    "博弈",
...
research = []
source = "未知"
summary = "面向未知环境的自主智能体可通过多游戏联合训练获得技能协同，从而提升可扩展的自主决策能力。"
paper = ""
code = ""
dataset = ""
link = ""
content = "该研究方向聚焦可扩展的自主提升，目标是构建能够在未知环境中智能决策的自主智能体。训练过程以游戏环境作为能力学习与验证载体。观察表明，结合多个游戏进行训练时，不同技能之间会产生协同效应，有望增强智能体的泛化能力。"
purpose = "- 探索以多游戏联合训练促进技能迁移与能力复用\n- 为未知环境中的自主决策和泛化提供训练思路\n- 将技能协同作为评估智能体自主提升能力的重要维度"
