...
dataset = ""
link = ""
content = "S3Gym 将自测试、自评判和自我改进作为相互耦合的能力进行评估，并将宽松的探索阶段与严格的留出测试阶段分离。基准包含七个文本游戏，并使用可执行的环境验证器判断智能体行为及结果。研究比较了直接使用历史记录的上下文学习、分数条件化的摘要记忆以及参数训练三种经验吸收路径。实验发现，自我改进并非自动发生：摘要适合提炼可复用策略，但在依赖精确状态信息时可能不如原始历史；参数训练虽可带来显著提升，也可能出现不稳定改进和严重负迁移。"
