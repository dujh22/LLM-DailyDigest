...
content = "LayerRecall将长视频记忆分为“检索什么”和“在哪些层使用”两个环节：紧凑摘要用于选择相关历史片段，选中片段的完整KV则仅注入预先识别出的记忆敏感层。其CHPM训练方法以长上下文冻结骨干作为教师，通过去噪预测匹配训练受限上下文学生，无需人工标注记忆分配标签。方法仅训练约165万参数、占5B骨干约0.033%的路由模块，在MemoBench和MovieBench上分别取得0.548和0.578的Overall成绩，同时基本保持VBench-Long质量。端到端生成时间仅由305.9秒增至309.4秒，路由器还可零训练迁移到LongLive和Self-Forcing，但需要使用目标骨干自身的记忆层配置。\n\n[合并自 2026-09-07 layerrecall]\nLayerRecall面向分块式自回归视频扩散中历史线索被近期缓存淘汰的问题，使曾经出现的主体、物体、场景和属性能够在长视频中保持一致。研究发现，视频DiT不同层对当前、近期和远期上下文存在不同偏好，因此该方法依据当前状态检索相关历史K/V，并仅将其注入骨干网络中的记忆敏感层。为减少对高质量长视频和显式记忆分配标签的依赖，作者提出跨时域预测匹配（CHPM），利用拥有长上下文的特权参考模型在预测空间监督有限记忆路由器。在100个多镜头提示上的实验中，该方法在MemoBench和MovieBench取得最佳综合结果，同时在VBench-Long上维持骨干模型表现，并展现跨骨干迁移能力和可忽略的推理开销。"
purpose = "- 为长视频生成中的角色身份、服装属性、物体和场景布局提供选择性远期记忆机制。\n- 表明扩大缓存或向所有网络层注入历史并非最优方案，分层记忆调度可减少运动突变与时间抖动。\n- CHPM提供了一种无需显式记忆标签、仅依赖冻结长上下文教师训练轻量路由器的路径。\n- 开源的训练、推理和评测链路可用于研究跨视频骨干的记忆迁移及低成本长程一致性增强。\n\n[合并自 2026-09-07 layerrecall]\n- 为长视频生成提供兼顾局部连续性与远期信息恢复的分层记忆机制。\n- 表明长程记忆不仅需要决定检索什么，还需要决定在哪些网络层使用检索结果。\n- CHPM可作为缺少长时域训练数据及记忆路由标注时的蒸馏式监督方案。\n- 记忆引导的自我纠正现象可用于修复跨镜头属性漂移，同时避免重置既有运动和场景结构。"
...
source = "arXiv"
summary = "ASPIRE 基准表明，在仅提供模糊目标和聚合分数的条件下，现有模型无论修改权重还是演化智能体脚手架，都难以稳定提升隐藏评测集表现。\n\n[合并自 2026-09-02 aspire]\nASPIRE 通过仅提供自然语言能力目标、隐藏下游任务，评测智能体自主定义学习目标并完成模型权重或脚手架演化的能力。\n\n[合并自 2026-09-07 aspire]\nASPIRE 通过隐藏下游任务，仅提供自然语言能力目标，评测智能体能否自主确定学习内容、更新方式与验证信号并实现模型或脚手架进化。"
...
content = "ASPIRE 将任务、数据和评分标准全部隐藏，只向智能体提供覆盖 6 个目标、共 520 道专家题目的聚合分数，以评估模型能否自主决定学习内容、改进方式与停止时机。实验显示，模糊目标使模型将大量计算预算用于解释目标，而非执行有效训练或能力更新，Claude Opus 4.8 与 GPT-5.6 的成绩均明显低于显式任务参考。权重自我进化实验中，绝大多数配置虽然能产出可评测 checkpoint，却无法稳定超过基线，说明训练完成并不代表能力提升。脚手架演化同样受窄验证集、错误代理目标和输出完整性问题影响，最佳 successor 仍落后于固定 Qwen-Agent 参考。\n\n[合并自 2026-09-02 aspire]\nASPIRE 是面向模糊目标驱动自进化的评测基准，只向智能体提供自然语言能力目标，而将下游评测任务隐藏。智能体需要自行解释目标、识别能力缺口、选择数据与更新方法、构造训练和验证信号，并决定何时开展评估。基准在统一交互环境中支持模型权重与智能体脚手架两类演化，并使用覆盖六项目标的 520 道专家编写题目进行隐藏评测。实验显示，当前智能体虽能完成训练和脚手架编辑闭环，但权重层面的收益稀疏且不稳定，最强演化脚手架仍落后于人工设计的 Qwen-Agent；数据错配和狭窄的自我评估还会导致局部收益无法迁移，持续搜索甚至可能抹去已有改进。\n\n[合并自 2026-09-07 aspire]\nASPIRE 是面向模糊目标驱动自我进化的交互式基准，只向智能体提供自然语言能力目标，并隐藏实际下游评测任务。智能体需要自行解释目标、识别能力缺口、选择训练数据与更新方法、构造训练及验证信号，并决定何时接受正式评估。该基准统一支持模型权重进化和智能体脚手架进化，最终使用覆盖六类目标的 520 道专家编写题目进行隐藏评测。实验显示，当前智能体虽能完成训练和脚手架编辑闭环，但权重层面的提升稀疏且不稳定，最强进化脚手架仍落后于人工设计的 Qwen-Agent；数据错配、狭窄自评和持续训练造成的能力退化是主要问题。"
purpose = "- 为递归自我改进研究提供更严格的评测设置，重点考察目标设定、验证设计和停止决策，而不只是训练流程能否运行。\n- 提醒自进化系统避免将聚合分数或窄验证集当作可靠代理目标，应加强隐藏集泛化、资格门槛和过程审计。\n- 表明模型权重更新与智能体脚手架编辑都需要更可靠的反馈规划、自我验证和能力提升判据。\n- 可与 S3Gym、PaperGym 等基准结合，用于研究缺乏明确指标时的自主目标发现与闭环验证。\n\n[合并自 2026-09-02 aspire]\n- 将自进化研究从优化人工明确指定的任务，推进到自主解释目标、选择学习内容和判断改进效果。\n- 可用于同时比较模型权重更新与智能体脚手架演化，分析两条技术路线的能力边界。\n- 提示自进化系统需要更可靠的目标分解、数据选择和外部验证机制，避免对狭窄自评信号过拟合。\n- 隐藏评测揭示了持续训练中的能力回退问题，可为演化过程的检查点保留、停止策略和验证棘轮设计提供依据。\n\n[合并自 2026-09-07 aspire]\n- 为研究开放式、模糊目标下的智能体自我进化提供统一评测环境。\n- 揭示局部自评提升不等于隐藏任务上的能力迁移，需要更可靠的验证信号与停止策略。\n- 可用于比较模型权重更新与脚手架演化两条路径，并研究数据选择、目标解释和灾难性遗忘问题。\n- 提示自进化系统应引入更广泛的验证集、能力回归检测和历史最佳状态恢复机制。"
...
source = "arXiv"
summary = "S3Gym 通过七个文本游戏联合评估 LLM 的自测试、自评判与自我改进能力，揭示不同经验利用方式具有显著的任务依赖性与负迁移风险。\n\n[合并自 2026-09-07 s3gym-llm]\nS3Gym 通过七个文本游戏评测 LLM 能否主动测试行为、判断交互经验，并将反馈转化为可执行且可迁移的改进策略。"
paper = "https://arxiv.org/abs/2608.31100"
code = ""
dataset = ""
link = ""
content = "S3Gym 将智能体自我改进拆分为 Self-Testing、Self-Judging 和 Self-Improvement 三项耦合能力，并区分宽松探索与严格的留出评估。基准覆盖七个带有可执行环境验证器的文本游戏，对比直接使用历史记录进行 ICL、分数条件化的摘要记忆以及参数训练三种经验利用路径。实验显示，自我改进并不会自动或稳定发生：摘要适合把经验压缩为可复用策略规则，而依赖精确状态信息的任务往往更适合保留原始历史。参数训练在部分任务上带来显著提升，但也可能产生不稳定改进和严重负迁移，说明识别成功行为并不等同于形成可执行、可迁移的策略。\n\n[合并自 2026-09-07 s3gym-llm]\nS3Gym 将自测试、自评判和自我改进作为相互耦合的能力进行评估，并将宽松的探索阶段与严格的留出测试阶段分离。基准包含七个文本游戏，并使用可执行的环境验证器判断智能体行为及结果。研究比较了直接使用历史记录的上下文学习、分数条件化的摘要记忆以及参数训练三种经验吸收路径。实验发现，自我改进并非自动发生：摘要适合提炼可复用策略，但在依赖精确状态信息时可能不如原始历史；参数训练虽可带来显著提升，也可能出现不稳定改进和严重负迁移。"
purpose = "- 为智能体是否真正具备从交互经验中持续改进的能力提供统一评测框架。\n- 提示经验压缩方式应与任务结构匹配，不能默认摘要记忆优于完整交互历史。\n- 将“识别成功行为”与“形成可执行、可迁移策略”区分开，有助于定位自我改进链路中的瓶颈。\n- 参数更新需要同时监控稳定性与跨任务负迁移，单项任务性能提升不足以证明可靠的自我改进。\n\n[合并自 2026-09-07 s3gym-llm]\n- 为智能体自我改进能力提供统一的交互式评测协议，而非仅将模型视为固定策略进行静态测试。\n- 可用于比较历史上下文、摘要记忆和参数更新三类经验学习机制的适用边界。\n- 揭示从反馈识别到策略内化之间的关键瓶颈，并帮助诊断负迁移与改进不稳定问题。\n- 为设计具备可靠闭环学习能力的智能体提供任务结构与经验表示方面的依据。"
...
research = []
source = "arXiv"
summary = "PLC-DPO利用校准后的策略—参考模型边际，将偏好样本在线路由为正常、翻转或平局信号，从而提升DPO在噪声和模糊标注下的鲁棒性。"
paper = "https://arxiv.org/abs/2608.30597"
code = ""
dataset = ""
link = ""
content = "DPO通常假设成对偏好标签可靠，但真实数据中常存在标签颠倒、偏好较弱及歧义，可能导致有害的策略更新。PLC-DPO以校准后的策略—参考模型边际作为在线证据，将每个样本的训练信号路由为正常、翻转或平局三种情况。该方法不只是过滤可疑样本，而是主动校正监督信号的方向与强度。在57个数据集—模型—基准组合中，PLC-DPO相对DPO取得60.5%的平均胜率，并在注入噪声、平局压力测试、人类分歧分析及自确认诊断中表现出稳定性。"
purpose = "- 为含错标、弱偏好和主观分歧的真实偏好数据提供更稳健的DPO训练方案。\n- 将偏好数据治理从简单过滤扩展为对监督方向和强度的在线校正。\n- 可用于降低噪声偏好引发的错误策略更新，并提升后训练阶段的可靠性。\n- clean、flip、tie三路信号设计可为其他奖励建模与偏好优化方法提供参考。"
