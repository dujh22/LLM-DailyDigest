...
source = "机器之心"
summary = "南京大学与南洋理工大学提出 VibeGame，通过 AI 原生游戏引擎、八角色对抗式 Agent 团队与免训练经验复用机制，实现从自然语言需求到可持续迭代游戏工程的闭环自动化开发。"
paper = ""
code = "https://github.com/tettethu/VibeGame"
dataset = ""
link = "https://mp.weixin.qq.com/s/aDRYXLMXpc_QyQHfrxwiYQ"
content = "VibeGame 将任务定义为 Prompt-to-Game Development，可根据自然语言及可选概念图生成完整、可玩的游戏工程，也支持通过自然语言持续修改已有项目。系统基于 Phaser 构建 AI 原生引擎，以类型化 JSON 表达场景、动画、碰撞和数值，并提供暂停、逐帧推进、动作注入、状态读取及截图验证能力。制作人、策划、美术、架构、程序、审计、试玩和验收八类 Agent 分工协作，通过生成者与检验者分离、意图对齐、并行开发、试玩审计和失败返工形成对抗式开发闭环。每次成功交付都会沉淀为骨架、组件和协作契约，后续经检索、适配与重新验证实现跨项目复用，无需更新模型参数即可持续进化。"
purpose = "- 为面向 Agent 的开发基础设施提供设计范例：相比为传统 GUI 引擎外挂接口，全文本、状态可读写、流程可暂停且结果可验证的环境更适合智能体操作。\n- 展示多智能体职责分离、交叉检查及独立审计、试玩与验收如何减少单一 Agent 自我评判造成的遗漏，为复杂软件工程的质量控制提供参考。\n- 将骨架、组件和协作契约作为可检索的非参数化经验记忆，为跨项目复用、提升后续任务起点及团队级自进化提供实现路径。\n- 可用于自然语言游戏创建、概念图驱动开发、存量游戏编辑和持续的人机协同迭代。\n- 逐帧试玩、独立审计和验收返工机制还可推广至其他具有实时交互与主观质量要求的软件开发场景。"
...
source = "Hugging Face Papers"
summary = "浙大与港大提出LayerRecall，通过按当前状态检索历史片段，并仅向记忆敏感的Video DiT层注入完整K/V，以极低参数量和推理开销提升自回归视频生成的长时域一致性。"
paper = "https://arxiv.org/abs/2608.28460"
code = "https://github.com/Yixuan-Ding-ZJU/LayerRecall"
dataset = ""
link = "http://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652723319&idx=3&sn=a74ffdf28545dd3c3c4c144b9baa81b6&chksm=f0f965ed0bf55748703cbc0961af38a431cc125d860d15679fc41ee8fca2bbe9f59f77ad192a&scene=0&xtrack=1#rd"
content = "LayerRecall针对分块式自回归视频扩散中历史线索易被近期缓存淘汰的问题，将长视频记忆拆分为“检索什么”与“在哪些层使用”：先以紧凑摘要筛选相关历史片段，再将其完整K/V注入预先识别的记忆敏感层，从而维持主体、物体、场景和属性的一致性。研究发现，Video DiT不同层对当前、近期和远期上下文具有不同偏好，因此选择性分层注入优于简单扩大缓存或向所有层写入历史。其跨时域预测匹配（CHPM）以冻结的长上下文骨干为教师，在预测空间通过去噪结果匹配监督受限上下文学生，无需高质量长视频、人工记忆分配标签或骨干微调，仅训练约165万参数、占5B骨干约0.033%的路由模块。在100个多镜头提示的实验中，LayerRecall在MemoBench和MovieBench上的Overall成绩分别达到0.548和0.578，同时基本保持VBench-Long质量，端到端生成时间仅从305.9秒增至309.4秒；路由器还可零训练迁移至LongLive和Self-Forcing，但需采用目标骨干自身的记忆层配置。"
purpose = "- 为长视频生成提供兼顾局部连续性与远期信息恢复的分层记忆机制，减少角色身份、服装属性、物体及场景布局的跨镜头漂移。\n- 表明长程记忆既要决定检索哪些历史信息，也要决定在哪些网络层使用；相比扩大缓存或全层注入，选择性调度有助于降低运动突变与时间抖动。\n- CHPM提供了一种在缺少长时域训练数据和显式记忆路由标签时，利用冻结长上下文教师训练轻量路由器的蒸馏式监督方案。\n- 记忆引导的自我纠正可用于修复跨镜头属性漂移，同时避免重置已有运动和场景结构。\n- 开源的训练、推理和评测链路可支持跨视频骨干的记忆迁移研究，以及低成本长程一致性增强。"
notes = "[LayerRecall：用于视频生成长时域一致性的状态条件记忆路由器（25 ▲）](https://huggingface.co/papers/2608.28460?utm_source=digest-papers&utm_medium=email&utm_campaign=2026-08-31)\n\n[合并自 2026-09-07 layerrecall]\n标题：角色离场再回来，AI就换了个人？浙大、港大开源轻量化记忆路由器\n来源：新智元（微信公众号）\n链接：http://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652723319&idx=3&sn=a74ffdf28545dd3c3c4c144b9baa81b6&chksm=f0f965ed0bf55748703cbc0961af38a431cc125d860d15679fc41ee8fca2bbe9f59f77ad192a&scene=0&xtrack=1#rd\n摘要："
...
source = "新智元"
summary = "Google DeepMind 联合理论计算机科学家，利用 Gemini 驱动的 AlphaEvolve 优化大规模科研程序，将矩阵乘法指数上界从约 2.371339 改进至 ω<2.371177。"
paper = "https://arxiv.org/abs/2608.16884v1"
code = ""
dataset = ""
link = "http://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652722026&idx=3&sn=cbbd3227761fa83f9396863d82dcc654&chksm=f05b9591fd498b86a27b1b611e4d3dcb89829323eace418c0a80027022bea8f8cf8dc1a5c5de&scene=0&xtrack=1#rd"
content = "Google DeepMind 与 Josh Alman、Virginia Vassilevska Williams 等研究者合作，重新形式化组合损失分析，将可处理的递归层级从 ℓ*=3 提升至 ℓ*=4，使非凸优化的参数规模由约 2.5 万扩展至约 700 万。系统采用张量表示、幽灵节点和分阶段并行，并结合 Adam、Softmax 参数化、Sinkhorn-Knopp 算法与隐式微分在 GPU 上高效搜索，贡献约 0.97×10⁻⁴ 的改进。随后，Gemini 驱动的 AlphaEvolve 通过进化方式改写和优化求解程序，在单 GPU 约 5 小时的运行中进一步贡献约 0.63×10⁻⁴ 的改进，最终得到 ω<2.371177 的新上界。该成果体现了人类负责构建理论框架、设定约束与验证路径，AI 负责大规模探索、程序优化和候选精炼的人机协作模式。"
purpose = "- 展示代码进化智能体能够优化科研算法本身，并在经典理论计算机科学问题上取得可严格验证的新纪录。\n- 说明即使缺乏全新的数学框架，通过扩大参数化规模、GPU 并行、可微优化和自动搜索优化器，仍可能推动已知边界取得实质性进展。\n- 为自动化算法发现提供人机分工范式：专家定义问题、约束、目标函数与验证器，智能体在可验证的搜索空间中持续改写程序并精炼候选方案。\n- 表明 GPU 并行优化、可微参数化与程序进化的结合，可用于数值算法、组合优化及其他具备明确目标函数和严格验证机制的大规模非凸研究问题。"
notes = "标题：DeepMind AlphaEvolve再破纪录：矩阵乘法指数40年最低！\n来源：新智元（微信公众号）\n链接：http://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652722026&idx=3&sn=cbbd3227761fa83f9396863d82dcc654&chksm=f05b9591fd498b86a27b1b611e4d3dcb89829323eace418c0a80027022bea8f8cf8dc1a5c5de&scene=0&xtrack=1#rd\n摘要：\n\n[合并自 2026-09-02 alphaevolve]\nEvolve应用｜DeepMind AlphaEvolve再破纪录：矩阵乘法指数40年最低！https://mp.weixin.qq.com/s/XZunT2-MO06tk4ToSrxP0A"


[[items]]
id = "papergym"
title = "PaperGym：以评分准则驱动科研计划生成与训练"
subtopic = "科研智能体"
topics = [
    "自动化科学研究",
    "模型训练",
...
source = "arXiv"
summary = "ASPIRE 表明，在仅提供模糊能力目标并隐藏下游任务的条件下，现有智能体虽能自主完成模型权重或脚手架的演化闭环，却难以在隐藏评测中实现稳定、可持续的能力提升。"
paper = "https://arxiv.org/abs/2608.31111"
code = ""
dataset = ""
link = "https://www.xiaohongshu.com/discovery/item/6a96609800000000260168b6?source=webshare&xhsshare=pc_web&xsec_token=CBuH0yJDknMBJ2yrxLfLGY54kf6jt9RCS2nbhIwhdBnu4=&xsec_source=pc_share"
content = "ASPIRE 是面向模糊目标驱动自我进化的交互式基准，仅向智能体提供自然语言能力目标，并以覆盖 6 类目标的 520 道专家题目进行隐藏评测。智能体需要自主解释目标、诊断能力缺口、生成或选择数据、构造训练与验证信号，并决定采用 SFT、GRPO、LoRA 等更新方式及何时停止。实验显示，多数配置虽然能够产出可评测 checkpoint 或完成脚手架编辑，但权重收益稀疏且不稳定，最强演化脚手架仍落后于人工设计的 Qwen-Agent。数据错配、狭窄自评、错误代理目标和持续搜索引发的能力退化，说明训练闭环成功并不等于能力闭环成立。"
purpose = "- 为开放式、模糊目标下的递归自我改进提供严格评测，重点考察目标操作化、数据选择、验证设计与停止决策。\n- 支持比较模型权重更新与智能体脚手架演化两条路线，分析其能力边界及失败模式。\n- 提醒研究者区分流程成功、局部峰值与隐藏任务上的稳定增益，避免对聚合分数或狭窄自评信号过拟合。\n- 自进化系统应引入更广泛的隐藏评测、能力回归检测、检查点保留与历史最佳状态回滚机制。"
notes = "标题：Aspire: Can Models Self-Evolve from Vague Goals?\n来源：arXiv\n链接：https://arxiv.org/abs/2608.31111\n摘要：Many important forms of human learning begin with a vague goal, such as \"become a better physicist\" or \"improve at research.\" Learners must interpret the goal, identify capability gaps, decide how to learn, and determine whether they have actually improved. In contrast, existing work on LLM self-evolution typically begins with tasks and evaluation metrics specified by humans, reducing self-evolution to optimizing an explicit objective rather than deciding what and how to learn. We introduce ASPIRE, a benchmark for vague-goal-driven self-evolution. ASPIRE provides only a natural-language capability goal while downstream evaluation tasks remain hidden. The agent must operationalize the goal by choosing data and update methods, constructing training and validation signals, and deciding when t\n\n[合并自 2026-09-02 aspire]\n68 【Aspire揭露真相：AI搞不定模糊目标自进化！ - RSILab | 小红书 - 你的生活兴趣社区】 😆 6GHzOWDc2gabV6y 😆 https://www.xiaohongshu.com/discovery/item/6a96609800000000260168b6?source=webshare&xhsshare=pc_web&xsec_token=CBuH0yJDknMBJ2yrxLfLGY54kf6jt9RCS2nbhIwhdBnu4=&xsec_source=pc_share\n\n[合并自 2026-09-07 aspire]\n[Aspire：模型能否从模糊目标中自我进化？（204 ▲）](https://huggingface.co/papers/2608.31111?utm_source=digest-papers&utm_medium=email&utm_campaign=2026-09-03)\n\n[合并自 2026-09-09 seed-agent]\nhttps://mp.weixin.qq.com/s/L6pY-3rDUy6pLQ96DcOFyg"


[[items]]
id = "llm-dataware"
title = "工业级 LLM 后训练：面向棕地维护的 Dataware 工程"
subtopic = "数据混合"
topics = [
    "模型训练",
    "数据工程",
...
source = "arXiv"
summary = "S3Gym 通过七个文本游戏联合评估 LLM 的自测试、自评判与自我改进能力，揭示不同经验利用方式存在显著的任务依赖性、改进不稳定性和负迁移风险。"
paper = "https://arxiv.org/abs/2608.31100"
code = ""
dataset = ""
link = ""
content = "S3Gym 将智能体自我改进拆分为相互耦合的 Self-Testing、Self-Judging 和 Self-Improvement 三项能力，并将宽松探索与严格的留出评估分离。基准覆盖七个文本游戏，使用可执行的环境验证器判断智能体行为及结果。研究比较了三种经验利用路径：直接基于完整历史记录进行上下文学习（ICL）、采用分数条件化的摘要记忆，以及通过参数训练吸收经验。实验表明，自我改进不会自动或稳定发生：摘要更适合将经验压缩为可复用的策略规则，而依赖精确状态信息的任务通常更适合保留原始历史；参数训练虽能在部分任务上显著提升性能，也可能导致改进不稳定和严重负迁移。结果说明，识别成功行为并不等同于形成可执行、可迁移的策略。"
purpose = "- 为智能体能否从交互经验中持续改进提供统一的交互式评测协议，而非仅将模型视为固定策略进行静态测试。\n- 可用于比较完整历史上下文、摘要记忆和参数更新三类经验学习机制的适用边界。\n- 提示经验表示与压缩方式应匹配任务结构，不能默认摘要记忆优于完整交互历史。\n- 区分“识别成功行为”与“形成可执行、可迁移策略”，有助于定位从反馈判断到策略内化的关键瓶颈。\n- 参数更新需要同时监控稳定性和跨任务负迁移，单项任务性能提升不足以证明可靠的闭环自我改进。"
notes = "标题：S3Gym: Can LLMs Turn Self-Testing and Self-Judging into Self-Improvement?\n来源：arXiv\n链接：https://arxiv.org/abs/2608.31100\n摘要：Large language models (LLMs) increasingly interact with external environments and accumulate substantial behavioral experience, yet existing agent benchmarks largely evaluate them as fixed policies. It therefore remains unclear whether an agent can actively test its behavior, judge the resulting experience, and use that experience to improve future decisions. We introduce \\textbf{S\\textsuperscript{3}Gym}, an interactive benchmark for evaluating LLM self-improvement through three coupled capabilities: \\textbf{Self-Testing}, \\textbf{Self-Judging}, and \\textbf{Self-Improvement}. S$^3$Gym separates permissive exploration from strict held-out evaluation and instantiates this protocol in seven text-based games with executable environment verifiers. We evaluate three pathways for incorporating in\n\n[合并自 2026-09-07 s3gym-llm]\n[S3Gym：LLMs 能否将自测试和自评判转化为自我改进？（26 ▲）](https://huggingface.co/papers/2608.31100?utm_source=digest-papers&utm_medium=email&utm_campaign=2026-09-03)"


[[items]]
id = "item-27"
title = "水印与掩蔽递归离散分布估计的极小极大界"
subtopic = "数据合成"
topics = [
    "数据合成",
    "数据工程",
...
source = "arXiv"
summary = "研究发现，在策略蒸馏（OPD）的增益主要源于抑制低概率尾部词元而非教师知识迁移，并据此提出无需教师监督、可实现显著自我改进的熵自适应方法 OPSA。"
paper = "https://arxiv.org/abs/2608.31046"
code = ""
dataset = ""
link = ""
content = "研究定量分析了在策略蒸馏（OPD）的教师监督，发现教师对学生生成轨迹的评分包含大量噪声，且教师模型越大，噪声越普遍。无论保留还是移除这些噪声监督，学生最终性能均较为接近；训练主要作用于低对数概率词元，固定负优势也能达到教师优势信号的效果，说明 OPD 更可能依赖概率分布重塑而非教师知识迁移。基于这一发现，作者提出无需教师的 On-Policy Self-Adaptation（OPSA），按照词元位置熵自适应分配负优势，抑制尾部词元并在头部词元间重新分配概率质量。基于 Qwen3-1.7B，OPSA 在 AIME24 上将 Avg@32 提升 35.41 分，比 OPD 高 16.77 分，并在三个基准上均使 Pass@32 提升至两倍以上。"
purpose = "- 揭示 OPD 可能主要是一种概率分布重塑机制，而非传统意义上的教师知识迁移，评估蒸馏方法时应区分教师知识贡献与训练动态本身的收益。\n- 表明复杂且可能失真的教师评分可由简单、自适应的内部学习信号替代，为移除昂贵教师模型、降低在策略训练成本提供依据。\n- OPSA 为推理模型提供了无需外部监督的在策略自我改进路径，可利用词元熵和低概率区域设计更稳定的训练信号，减少对噪声奖励或优势估计的依赖。\n- 提示教师规模增大不必然改善监督质量，设计蒸馏系统时应专门审计教师在学生轨迹上的噪声。"
notes = "标题：Does On-Policy Distillation Really Distill? From Noisy Teacher to Self-Improvement\n来源：arXiv\n链接：https://arxiv.org/abs/2608.31046\n摘要：On-policy distillation (OPD) offers dense token-level supervision as an alternative to the sparse outcome-level advantages of reinforcement learning with verifiable rewards (RLVR). However, the teacher scores student-generated trajectories that are inherently off-policy for it, so the reliability of its supervision, and hence the source of the student's improvement, remains unclear. We quantitatively analyze teacher supervision during OPD training and find substantial noise whose prevalence increases with teacher scale. Surprisingly, the student policy is insensitive to such noise, converging to comparable performance regardless of whether noisy supervision is retained or removed. Does OPD distill at all? By analyzing what drives its gains, we find that learning concentrates on low log-pro\n\n[合并自 2026-09-02 item-9]\n[On-Policy Distillation Really Distill? From Noisy Teacher to Self-Improvement（100 ▲）](https://huggingface.co/papers/2608.31046?utm_source=digest-papers&utm_medium=email&utm_campaign=2026-09-01)"


[[items]]
id = "pde"
title = "以训练分布归纳偏置学习可接受 PDE 假设的几何结构"
subtopic = "数据合成"
topics = [
    "科学发现",
    "数据合成",
