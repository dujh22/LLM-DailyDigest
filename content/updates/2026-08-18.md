...
content = "Spellcaster先将用户描述拆解为游戏规则、角色能力、关卡目标、胜负条件、敌人行为和关键数值，再由六个专用Agent协同生成游戏原型。Rule、Level和Asset Agent分别处理规则、关卡与素材，Playability和Simulation Agent通过实际运行验证关键路径、核心交互及必死局，Repair Agent负责定位并局部修复问题。系统支持通过对话持续调整游戏速度、敌人、难度、关卡、数值和视觉风格，无需每次从头生成，约15分钟即可产出常见类型的可玩原型。团队下一阶段计划引入世界模型，根据玩家动作、角色状态与交互历史实时预测后续变化，并直接生成画面和反馈，逐步摆脱传统代码与游戏引擎管线。"
purpose = "- 将AI游戏生成的评价标准从“代码可运行”扩展到“玩法可完成、交互可持续”，形成覆盖生成、试玩、检查与修复的闭环验证范式。\n- 展示多Agent按规则、关卡、素材、仿真和修复分工的编排方式，降低复杂生成任务的问题定位与迭代成本。\n- 可用于独立游戏的玩法验证、互动内容原型制作，以及非程序员和内容创作者的低门槛创作。\n- 启示生成系统应保留逻辑自洽的意外玩法，而非仅依据预设规则机械删除偏差。\n- 为游戏生成从静态项目生产、代码驱动管线转向世界模型实时推演和响应玩家的动态交互系统提供产品路径。"
...
source = "arXiv"
summary = "ClawGym II 提出统一的黑箱强化学习框架，通过沙箱并发执行、模型边界代理与前缀树轨迹重建，稳定扩展复杂异构 Agent Harness 上的长程任务训练。"
...
research = []
source = "arXiv"
summary = "受控实验揭示同策略蒸馏主要迁移推理行为、其泛化取决于师生模型是否同源且多教师蒸馏存在能力跷跷板效应；OmniAssistBench 则通过逆向构建多轮视频交互评测，暴露出现有实时全模态助手在视觉提示识别、上下文保持和响应时机判断上的不足。"
paper = "https://arxiv.org/abs/2608.16647"
code = ""
dataset = ""
link = ""
content = "OPD 研究通过控制域内分布偏移、跨领域迁移和多教师设置等因素发现，蒸馏主要传递教师的推理行为而非特定答案，训练样本难度和教师是否答对的影响较小，失败轨迹也能提供有效监督。师生来源关系是泛化的关键：同源组合可实现跨语言、跨推理长度和跨领域迁移，异源组合通常只能拟合训练分布；多位领域教师的影响又难以通过提示路由完全隔离，会随混合比例形成能力此消彼长的跷跷板。OmniAssistBench 面向持续感知并实时指导用户完成目标的 Omni-LLM，从互联网视频反推用户目标、切分多轮片段并提供源视频路线先验，以控制交互路径偏差，数据构建耗费超过 1000 个专家工时。评测中 Gemini-3-Pro 得分为 66.4/100，Qwen3-Omni-Instruct 得分为 51.2；现有模型通常能理解用户输入，但容易回答错误或不完整，尤其不擅长识别手势等视觉提示、保持多轮历史上下文，以及等待目标事件出现后适时响应。"
purpose = "- 为 OPD 提供覆盖域内分布偏移、跨语言、跨推理长度、跨领域迁移及多教师设置的实验框架，避免仅凭单一领域或近训练分布基准判断泛化能力。\n- 选择教师—学生组合时应重点考虑模型来源关系；若追求广泛迁移，应优先考虑同源模型，而不能只比较教师自身的任务准确率。\n- 蒸馏数据筛选不必过度强调题目难度或教师答对率，因为失败轨迹中的推理行为仍可能促进能力迁移。\n- 多教师蒸馏不能仅依赖领域提示隔离教师影响，需要显式评估不同混合比例下各项能力的增益、干扰与退化。\n- OmniAssistBench 为实时视频助手和 Omni-LLM 提供了比静态视频问答更贴近真实使用场景的多轮交互评测，并可将视觉提示识别、上下文保持和响应时机拆分为独立能力维度，用于模型诊断与定向训练。\n- 从互联网视频逆向构造交互轨迹可缓解真实人机交互视频稀缺的问题；评测结果也表明，可靠的实时助手不仅要正确回答，还需持续维护状态并判断何时响应。"
notes = "标题：Every Coin Has Two Sides: On the Dual Nature of Generalization in On-Policy Distillation of Large Language Models\n来源：arXiv\n链接：https://arxiv.org/abs/2608.16647\n摘要：On-policy distillation (OPD) transfers teacher capabilities by supervising trajectories sampled from the student's own policy, yet its generalization behavior remains poorly understood, as most studies evaluate OPD on a single domain and on benchmarks close to the training data. We present a controlled study that varies one generalization factor at a time, from in-domain distribution shifts to cross-domain transfer and the multi-teacher setting. We find that OPD transfers a teacher's reasoning behavior rather than its answers to particular problems: training difficulty barely matters, and even problems the teacher never solves are useful. Transfer depends strongly on the origin relationship between teacher and student: same-origin pairs bring the student close to the teacher across languag\n\n[合并自 2026-08-24 opd]\nhttps://huggingface.co/papers/2608.16647\n\n[合并自 2026-08-25 omniassistbench]\n[每枚硬币都有两面：大型语言模型在线策略蒸馏中泛化的双重性质（15 ▲）](https://huggingface.co/papers/2608.16647?utm_source=digest-papers&utm_medium=email&utm_campaign=2026-08-24)"
