...
source = "机器之心"
summary = "MirroS 联合清华、北大、Berkeley、MIT、英伟达等 15 家学术与产业机构发布 HarnessEval，以可规划、路由、拆解、验证并生成可追溯证据树的智能体工作流，将静态 Benchmark 升级为可持续进化的可执行评测系统。"
paper = ""
code = "https://github.com/mirros-lab/harnesseval-w"
dataset = ""
link = "https://mp.weixin.qq.com/s/T_fBh7p82OHaKw75oq-5cQ"
content = "HarnessEval 首次将 Harness 执行框架引入评测，主 Agent 通过 Plan、Route、Decompose、Verify 四个阶段，按案例选择技能、调用工具、拆解可测问题，并派生专业子 Agent 搜集、交叉验证和审计证据。首个开源实现 HarnessEval-W 面向交互式世界模型，围绕观测质量、状态转移正确性和世界持续性三条主轴及九项技能动态组织评估，并计划扩展至代码、搜索和机器人等复杂场景。系统不仅负责评分，还参与候选场景生成与有效性验证，使 Benchmark 从冻结数据集转变为持续生产、质检和更新任务的系统。评测结果同时包含量化分数与层级式 Evidence Tree，记录评测目标、工具调用、证据来源和结论形成过程，以支持诊断、审计与复核。该框架还将评测视为递归式自我改进闭环中的反馈系统，可通过扩展和版本化 Skill Library、增加测试时计算，持续适应模型能力与失败模式的变化。"
purpose = "- 为复杂 Agent、世界模型及交互式生成系统提供按案例动态编排的评测流程，并可随任务难度扩展推理、工具调用和验证深度，减少固定 Rubric 导致的无关检查与关键遗漏。\n- 通过 Evidence Tree 提升评分的可解释性、可复现性、可审计性与可复核性，帮助区分失败源于模型、工具、环境还是评测器，并降低弱裁判限制前沿模型评估的风险。\n- 将行业评测能力沉淀为可复用、可组合、可版本化的 Skill Library，结合子智能体分工和证据审计，快速补充针对新失败模式的验证能力，并扩展至代码、搜索、机器人等领域。\n- 支持候选场景生成、有效性验证、任务质检与持续更新，使 Benchmark 从静态数据集演变为可持续运行的评测基础设施。\n- 将评测器纳入测试时扩展与递归自我改进闭环，使其能够识别自身技能缺口、引入工具或新技能后重新验证，并降低围绕单一分数优化造成的奖励作弊与评测盲区风险。"
