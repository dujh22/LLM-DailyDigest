...
research = []
source = "arXiv"
summary = "Chain-of-Experience 让 LLM 在测试时积累自反馈与环境反馈形成持续改进循环，在提升整体表现 5.6% 的同时降低 19% API 成本。"
paper = "https://arxiv.org/abs/2608.18027"
code = ""
dataset = ""
link = ""
content = "论文提出 Chain-of-Experience（CoE），使模型通过反复交互积累经验轨迹，并在推理阶段形成超越零样本推理的持续改进循环。研究结合模型自反馈、答案正确性和公开编程测试通过率等反馈，在数学、编程与知识任务上评估 GPT-5、Gemini-2.5 Pro、Claude-4.5 Sonnet 等 8 个模型。CoE 相比无反馈基线取得稳定提升，整体性能提高 5.6%，API 成本降低 19%，且每个令牌带来的准确率高于已有测试时策略。互补反馈通道可以进一步增益，基础能力更强的模型通常也具有更大的改进潜力；多数收益出现在早期迭代中，模型对较弱或伪反馈仍表现出一定鲁棒性。"
purpose = "- 为无需更新模型参数的测试时持续学习提供通用框架。\n- 可将自反馈与可验证环境信号组合，用于数学推理、代码生成和知识问答系统。\n- 提示实际部署应优先利用早期迭代，并按收益及时停止，以兼顾准确率、令牌消耗和 API 成本。\n- 可用于构建评价模型“从交互经验中改进”能力的新型动态评测。"
...
research = []
source = "arXiv"
summary = "研究发现，准确率无法反映低资源语言后训练的关键变化：SFT 能建立希腊语推理习惯，而可验证奖励强化学习可进一步修复格式违约、通道泄漏等行为缺陷。"
paper = "https://arxiv.org/abs/2608.17744"
code = ""
dataset = ""
link = ""
content = "研究对三款前沿混合专家模型进行希腊语推理微调，发现仅更换随机种子就会造成 7.7 分的准确率波动，超过所有数据和训练配方带来的效果。基础模型在 1,000 条轨迹中从不使用希腊语思考，而 SFT 后约 98% 的样本会使用问题对应语言推理，同时改善语法流畅度，并基本保持通用能力。论文提出六个带长度相关性门控的行为测量维度，并通过对照实验识别出六类测量工具失效。SFT 仍存在格式跳过、答案泄漏至推理通道和指令服从不足等问题；采用预注册的可验证奖励强化学习后，格式回退率从 24% 降至 2.5%，泄漏率从 3.5% 降至 0%，且希腊语推理习惯得以保留。"
purpose = "- 提醒低资源语言模型评测不能只看最终准确率，还应检查推理语言、语法质量、格式遵循和通道边界。\n- 可将 SFT 用于建立目标语言的稳定推理习惯，再用可验证奖励强化学习针对具体行为缺陷进行修复。\n- 评测指标应加入随机种子对照、随机奖励对照和输出长度相关性门控，避免把噪声或长度效应误判为能力提升。\n- 该方法及其测量工具可迁移到其他低资源语言，用于提升推理过程的可读、可审计与可纠正性。"
...
source = "arXiv"
summary = "Agent Lightning v1.0 将部署时智能体脚手架直接接入强化学习后训练，并以轻量、可复现流程显著提升编码智能体在 SWE-bench Verified 上的表现。"
paper = "https://arxiv.org/abs/2608.17528"
code = ""
dataset = ""
link = ""
content = "Agent Lightning v1.0 提出“脚手架式智能体强化学习”范式，由部署侧脚手架负责工具、上下文、控制流及环境交互，训练器仅观测 LLM 请求—响应序列。框架通过 LLM endpoint proxy 将任意智能体接入强化学习训练，整体实现约 3,500 行代码，并支持指令遵循、搜索和编码智能体。该范式需要专门处理重分词、样本合并、优势计算、损失归一化和后端调度等稳定性问题。使用约 6,000 条训练样本和有限算力，强化学习将 Qwen3.5-9B 在 SWE-bench Verified 上的成绩从 41.8% 提升至 56.4%，绝对提升 14.6 个百分点。作者同时发布了完整编码智能体强化学习工作流与训练脚本，以支持复现实验。"
purpose = "- 为现有智能体脚手架接入强化学习后训练提供低侵入、可复用的工程路径。\n- 表明部署侧控制流与训练侧优化可以解耦，有利于复用真实运行环境中的工具链和上下文管理逻辑。\n- 可作为研究重分词、轨迹合并、优势归因、损失归一化及异构后端调度问题的轻量试验平台。\n- 证明中等规模模型可借助少量样本和适度算力显著提升真实软件工程任务能力。"
