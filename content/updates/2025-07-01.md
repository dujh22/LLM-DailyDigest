...
research = []
source = "微信公众号"
summary = "PAROAttention 利用视觉注意力的局部性，通过离线 Token 重排、稀疏化和低比特量化，在保持模型性能的同时提升视觉生成模型的推理效率。"
paper = "https://arxiv.org/abs/2506.16054"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/2ZWAdLD-_XdOz3kL3GRNjw"
content = "清华大学与字节跳动提出 PAROAttention，一种面向视觉生成模型的模式感知注意力优化方案。该方法通过硬件友好的离线 Token 重排统一注意力模式，并据此设计针对性的稀疏计算与量化方法。结合高效 CUDA 系统实现，方案可支持 5 倍注意力稀疏和 4 比特量化，同时较好地保持生成性能。项目主页：https://a-suozhang.xyz/paroattn.github.io/"
purpose = "- 利用视觉注意力的局部性，降低视觉生成模型的计算与显存开销\n- 通过离线重排统一不规则注意力模式，使其更适合 GPU 高效执行\n- 为稀疏注意力与低比特量化的协同优化提供硬件友好的实现思路"
notes = "2. 2025-07-01 10:50:12 Tuesday ｜ 用好视觉Attention局部性，清华、字节提出Token Reorder，无损实现5倍稀疏、4比特量化 https://mp.weixin.qq.com/s/2ZWAdLD-_XdOz3kL3GRNjw\n\n提出了一种简单且硬件友好的离线 “Token重排” 方案以实现注意力模式的统一化，并设计了针对性的稀疏与量化方法，配合高效的 CUDA 系统设计，展现了更优异的算法性能保持与硬件效率提升\n\n论文标题：PAROAttention: Pattern-Aware ReOrdering for Efficient Sparse and Quantized Attention in Visual Generation Models\n\n论文链接：https://arxiv.org/abs/2506.16054\n\n项目主页：https://a-suozhang.xyz/paroattn.github.io/"


[[items]]
id = "seqpo-simt"
title = "SeqPO-SiMT：序贯策略优化让同传性能直逼离线翻译"
subtopic = "同声传译"
topics = [
    "多模态",
    "强化学习",
...
research = []
source = "公众号"
summary = "SeqPO-SiMT将同声传译建模为序贯决策过程，通过优化完整决策序列，在控制延迟的同时显著提升翻译质量。"
paper = "https://arxiv.org/pdf/2505.20622"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/laRnBuuTfF-olWKv42TAgw"
content = "SeqPO-SiMT是一种面向同声机器翻译的序贯策略优化方法，相关论文入选ACL 2025。该方法将翻译内容与输出时机统一建模为序贯决策过程，不再局限于优化单个局部动作。通过对完整决策序列进行优化，模型能够更好地平衡翻译质量和响应延迟。实验显示，其性能可接近同等规模的离线翻译模型，并在部分指标上实现超越。"
purpose = "- 改善AI字幕和实时翻译“慢半拍”的问题，提升会议、直播及跨语言交流体验\n- 为需要同时权衡输出质量与响应延迟的流式多模态系统提供序贯决策思路\n- 说明策略优化方法有望缩小在线模型与离线模型之间的性能差距"
notes = "2. 2025-07-01 10:56:55 Tuesday ｜ ACL 2025 | AI字幕慢半拍，不知道大家在笑什么？新方法让同传性能直逼离线翻译 https://mp.weixin.qq.com/s/laRnBuuTfF-olWKv42TAgw\n\n该方法**将同传任务巧妙地建模为序贯决策过程，通过优化完整的决策序列，显著提升了翻译质量，同时有效控制了延迟，其性能直逼、甚至在某些方面超越了同等大小的离线翻译模型。**\n\n* 论文标题: SeqPO-SiMT: Sequential Policy Optimization for Simultaneous Machine Translation\n* 论文链接：https://arxiv.org/pdf/2505.20622"


[[items]]
id = "embodied-planner-r1-llm"
title = "Embodied Planner-R1：通过强化学习释放 LLM 的具身任务规划能力"
subtopic = "具身规划"
topics = [
    "强化学习",
    "具身智能",
...
research = []
source = "公众号"
summary = "IDEA-CVR 提出的 Rex-Thinker 将逻辑推理链引入视觉指代检测，通过分步推理与证据验证提升检测准确率、可解释性及不确定性识别能力。"
paper = "https://arxiv.org/abs/2506.04034"
code = "https://github.com/IDEA-Research/Rex-Thinker"
dataset = ""
link = "https://mp.weixin.qq.com/s/I0YU0lkrkJ7_bNLdYdY72w"
content = "IDEA-CVR 提出 Rex-Thinker，将思维链机制引入视觉指代物体检测任务。模型能够围绕文本指令进行多跳推理，分步定位并验证相关视觉证据，而非直接输出检测结果。实验显示，该方法在权威测评中提升了检测准确率，同时增强了推理过程的可解释性。模型还具备一定的认知边界判断能力，可对证据不足或无法确定的情况作出更可靠的响应。"
purpose = "- 为指代检测、视觉问答等任务提供显式多跳推理范式\n- 利用可追踪的证据链提升视觉模型的可解释性与可验证性\n- 为构建能够识别不确定性、避免盲目作答的多模态系统提供参考\n- 可通过开源代码与在线 Demo 复现并评估模型能力"
notes = "2025-07-01 11:06:31 Tuesday ｜ 会“思考”的目标检测模型来了！IDEA提出Rex-Thinker：基于思维链的指代物体检测模型，准确率+可解释性双突破https://mp.weixin.qq.com/s/I0YU0lkrkJ7_bNLdYdY72w\n\nIDEA 提出全新解决方案 Rex-Thinker ，首次将人类思维中的 “逻辑推理链” 引入视觉指代任务，让 AI 像人一样分步思考、验证证据，在权威测评中不仅准确率显著提升，更展现出强大的 “知之为知之” 能力！\n\n项目主页：\n\nhttps://rexthinker.github.io/ \n\n在线 Demo：\n\nhttps://huggingface.co/spaces/Mountchicken/Rex-Thinker\n\nDemo 论文地址：\n\nhttps://arxiv.org/abs/2506.04034\n\n开源代码：https://github.com/IDEA-Research/Rex-Thinker\n\n投稿人：Qing Jiang\n\n投稿团队：IDEA-CVR\n\n## 多跳推理"


[[items]]
id = "astro"
title = "ASTRO：通过上下文反思与回溯训练语言模型推理"
subtopic = "反思回溯"
topics = [
    "推理",
    "强化学习",
...
research = []
source = "微信公众号"
summary = "Sapient Intelligence 提出的分层推理模型 HRM 以双层循环架构实现高计算深度，无需预训练、CoT 数据或中间过程监督，便在复杂数独和大型迷宫等任务上取得近乎完美的表现。"
paper = "https://arxiv.org/abs/2506.21734"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/PousJsp2TP7cTUTUwtf6ZA"
content = "Sapient Intelligence 的研究者受人脑分层处理与多时间尺度机制启发，提出了分层推理模型（HRM）。该模型包含两个相互依赖的循环模块：高级模块负责缓慢、抽象的规划，低级模块负责快速、细致的计算。HRM 可在单次前向传递中执行顺序推理，并在无需显式监督中间过程的情况下保持训练稳定性和计算效率。实验显示，它在复杂数独求解和大型迷宫最优路径搜索等挑战性任务上取得了近乎完美的性能。"
purpose = "- 为无需 CoT 标注和过程监督的深度推理提供新架构思路\n- 通过多时间尺度模块分工兼顾全局规划与局部计算\n- 可用于数独、迷宫寻路及其他需要长程规划的组合推理任务\n- 展示循环计算架构替代显式长推理链的潜力"
notes = "1. 2025-07-01 11:12:26 Tuesday ｜\n\nhttps://mp.weixin.qq.com/s/PousJsp2TP7cTUTUwtf6ZA\n\n该模型无需预训练或 CoT 数据即可运行，但在包括复杂数独谜题和大型迷宫中最优路径查找在内的挑战性任务上却取得了 **近乎完美的性能** 。\n\n近日，受到人脑分层和多时间尺度处理机制启发，来自 Sapient Intelligence 的研究者提出了分层推理模型（HRM），这是一种全新循环架构，能够在保持训练稳定性和效率的同时，实现高计算深度。\n\n具体来说，HRM 通过两个相互依赖的循环模块，在单次前向传递中执行顺序推理任务，而无需对中间过程进行明确的监督：其中一个高级模块负责缓慢、抽象的规划，另一个低级模块负责处理快速、细致的计算。\n\n论文：Hierarchical Reasoning Model\n\n论文链接：https://arxiv.org/abs/2506.21734"


[[items]]
id = "llm-2"
title = "数学推理训练能否提升 LLM 的通用能力？推理能力的可转移性研究"
subtopic = "推理迁移"
topics = [
    "推理",
    "强化学习",
...
research = []
source = "百度官方公众号"
summary = "百度正式开源文心大模型 4.5 系列，并同步提供 API 服务及技术报告。"
paper = "https://yiyan.baidu.com/blog/publication/ERNIE_Technical_Report.pdf"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/jG0R66Uq_6kFwajb7XKM3w"
content = "百度官宣文心大模型 4.5 系列正式开源，进一步开放其核心模型能力。开发者除可获取开源模型外，还能通过 API 服务快速接入文心大模型。官方同步发布了 ERNIE 技术报告，介绍模型相关技术与能力。此次发布有助于降低企业和开发者使用国产大模型的门槛。"
purpose = "- 为开发者提供可自主部署和二次开发的国产开源大模型\n- 通过 API 服务支持应用快速集成与上线\n- 技术报告可用于了解 ERNIE 系列的模型设计与能力表现"
notes = "5. 2025-07-01 11:23:45 Tuesday ｜ 百度官宣文心大模型4.5系列正式开源，还同步提供API服务 https://mp.weixin.qq.com/s/jG0R66Uq_6kFwajb7XKM3w\n\n报告地址：https://yiyan.baidu.com/blog/publication/ERNIE_Technical_Report.pdf"


[[items]]
id = "embedded-7b-pro-moe-72b"
title = "华为开源盘古 Embedded 7B 与盘古 Pro MoE 72B 模型"
subtopic = "开源模型"
topics = [
    "模型开发",
    "人工智能",
...
research = []
source = "华为盘古（微信公众号）"
summary = "华为正式开源盘古 Embedded 7B 稠密模型、盘古 Pro MoE 72B 混合专家模型及昇腾原生高性能推理技术。"
paper = ""
code = "https://gitcode.com/ascend-tribe"
dataset = ""
link = "https://mp.weixin.qq.com/s/v1NNVaH9oDufqkrkyVLnVw"
content = "华为宣布开源盘古 Embedded 70 亿参数稠密模型，以及盘古 Pro MoE 720 亿参数混合专家模型。模型面向昇腾平台进行原生适配，并同步开放相关模型推理技术。其核心技术包括分组混合专家 MoGE 算法、自适应快慢思考合一机制，以及覆盖全链路的高性能推理系统优化。"
purpose = "- 为昇腾生态提供可直接研究、适配和部署的开源基础模型\n- 借助 MoGE 架构探索大规模混合专家模型的训练与推理效率优化\n- 为统一快速响应与深度推理能力提供新的模型设计参考\n- 推动国产算力平台上的大模型工程化与高性能部署"
notes = "9. 2025-07-01 10:44:45 Tuesday ｜ 华为正式宣布开源盘古 70 亿参数的稠密模型「 **盘古 Embedded** 」、盘古 Pro MoE 720 亿参数的混合专家模型（参见机器之心报道：[华为盘古首次露出，昇腾原生72B MoE架构，SuperCLUE千亿内模型并列国内第一](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw==&mid=2650971050&idx=1&sn=93a499f2a2bcb83302ad201b8d193bda&scene=142#wechat_redirect) ）和基于昇腾的模型推理技术。\n   开源链接：https://gitcode.com/ascend-tribe\n\nhttps://mp.weixin.qq.com/s/v1NNVaH9oDufqkrkyVLnVw\n\n更重要的是，这些模型采用了一些领先的技术来实现高效的训练和推理，比如分组混合专家 MoGE 算法、自适应快慢思考合一以及全链路的高性能推理系统优化。"


[[items]]
id = "black-forest-labs-flux-1-kontext-dev"
title = "Black Forest Labs 开源图像编辑模型 FLUX.1 Kontext[dev]"
subtopic = "图像编辑"
topics = [
    "模型开发",
    "多模态",
...
research = []
source = "微信公众号"
summary = "Black Forest Labs 开源旗舰图像模型 FLUX.1 Kontext[dev]，主打图像编辑并支持在消费级芯片上运行。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/Cu-58gySRJ0-bWCwO8ViuQ"
content = "Black Forest Labs 宣布开源旗舰图像模型 **FLUX.1 Kontext[dev]**。该模型专为图像编辑场景打造，可基于上下文对图像进行修改。其运行门槛较低，能够直接部署在消费级芯片上。"
purpose = "- 用于本地化图像编辑、内容创作与视觉应用开发\n- 降低高性能图像生成和编辑模型的部署门槛\n- 为消费级硬件上的多模态模型应用提供新选择"
notes = "2025-07-01 11:20:23 Tuesday ｜Black Forest Labs刚刚宣布开源旗舰图像模型 **FLUX.1 Kontext[dev]** ，专为图像编辑打造，还能直接在消费级芯片上运行。 https://mp.weixin.qq.com/s/Cu-58gySRJ0-bWCwO8ViuQ"


[[items]]
id = "2025-ai-ai"
title = "2025年AI现状报告：AI构建者手册"
subtopic = "行业报告"
topics = [
    "人工智能",
    "人工智能应用",
...
research = []
source = "微信公众号"
summary = "ICONIQ发布《The AI Builders Playbook 2025》，梳理2025年AI行业现状及AI产品建设趋势。"
paper = "https://cdn.prod.website-files.com/65d0d38fc4ec8ce8a8921654/685ac42fd2ed80e09b44e889_ICONIQ%20Analytics_Insights_The_AI_Builders_Playbook_2025.pdf"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/XKHqP-dDIaK0ny8iIJK9aA"
content = "ICONIQ发布了《The AI Builders Playbook 2025》，聚焦2025年AI行业现状与产品建设实践。报告从AI构建者视角总结市场发展和落地趋势，可用于了解行业阶段性变化。原文由微信公众号转载或解读，并附有完整报告PDF链接。"
purpose = "- 跟踪2025年AI行业与产品落地趋势\n- 为AI产品规划、市场分析和战略决策提供参考\n- 了解AI构建者面临的机会与实践方向"
notes = "2. 2025-07-01 11:35:41 Tuesday ｜ 2025年AI现状报告 https://mp.weixin.qq.com/s/XKHqP-dDIaK0ny8iIJK9aA\n\n报告链接：https://cdn.prod.website-files.com/65d0d38fc4ec8ce8a8921654/685ac42fd2ed80e09b44e889_ICONIQ%20Analytics_Insights_The_AI_Builders_Playbook_2025.pdf"


[[items]]
id = "7-1600-ai"
title = "7万个模型、1600万开发者：魔搭已建成中国最大AI开源社区"
subtopic = "开源社区"
topics = [
    "人工智能",
    "智能体工具",
...
research = []
source = "微信公众号"
summary = "魔搭社区已汇聚约7万个模型和1600万开发者，成为中国规模最大的AI开源社区。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/nAa4WcjcQz93neQHuMvo6w"
content = "魔搭社区已汇聚约7万个AI模型，覆盖多种任务与应用场景。平台注册开发者达到1600万，形成了较大规模的模型共享与开发生态。其发展表明，中国AI开源基础设施正从模型托管平台逐步演变为连接模型、工具和开发者的综合社区。"
purpose = "- 为开发者提供集中获取、体验和复用开源模型的平台\n- 降低AI模型开发与应用落地的门槛\n- 展示中国本土AI开源生态的规模化发展趋势"
notes = "3. 2025-07-01 11:30:31 Tuesday ｜ 7万个模型、1600万开发者，魔搭已建成中国最大AI开源社区 https://mp.weixin.qq.com/s/nAa4WcjcQz93neQHuMvo6w"


[[items]]
id = "ai-aq"
title = "真·全民 AI 健康管家来了！实测蚂蚁 AQ"
subtopic = "医疗人工智能应用"
topics = [
    "人工智能",
    "人工智能应用",
...
research = []
source = "微信公众号"
summary = "蚂蚁推出 AI 健康管家 AQ，可通过连续追问、药品识别和皮肤图像分析提供健康服务，并连接医院及智能硬件。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/FufBkoQv5Csk9FiG3-GaTQ"
content = "蚂蚁 AQ 定位为面向大众的 AI 健康管家，支持围绕健康问题进行连续追问。用户可借助图像能力识别药品、初步了解皮肤问题，降低获取健康信息的门槛。该产品还能连接医院服务与健康硬件，将咨询、就医和健康数据管理串联起来。相关能力主要用于健康辅助，不应替代专业医生的诊断与治疗。"
purpose = "- 展示大模型在大众健康咨询与多模态识别场景中的产品化路径\n- 通过连接医院和智能硬件，探索 AI 健康服务闭环\n- 医疗应用需重点关注诊断边界、隐私保护与信息准确性"
notes = "4. 2025-07-01 11:25:06 Tuesday｜真·全民AI健康管家来了！实测蚂蚁AQ：追问识药看皮肤，还能连医院接硬件  https://mp.weixin.qq.com/s/FufBkoQv5Csk9FiG3-GaTQ"


[[items]]
id = "deep-video-discovery"
title = "微软推出长视频探索智能体 Deep Video Discovery"
subtopic = "视频理解"
topics = [
    "智能体",
    "多模态",
...
research = []
source = "微信公众号"
summary = "微软提出 Deep Video Discovery 智能体，通过视频分段、自主规划和工具调用逐步检索信息，在多个长视频理解基准上取得领先成绩。"
paper = "https://arxiv.org/pdf/2505.18079"
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/zIl8FSqWXbdwOaMPAR1uUA"
content = "Deep Video Discovery（DVD）将长视频切分为较短片段，并把这些片段构成的内容空间视为智能体可探索的环境。系统利用大语言模型理解问题、开展推理并自主制定搜索计划。智能体能够选择合适的工具及调用参数，从视频环境中逐步获取证据，最终生成答案。该方法在多个长视频理解基准上取得领先表现。"
purpose = "- 为长视频问答和内容检索提供可扩展的智能体方案。\n- 展示将复杂多模态任务转化为环境探索与工具调用过程的可行性。\n- 可用于视频档案分析、影视内容检索、教育视频问答及监控视频理解。"
notes = "2025-07-01 10:46:19 Tuesday ｜ 微软推出深度视频探索智能体，登顶多个长视频理解基准 https://mp.weixin.qq.com/s/zIl8FSqWXbdwOaMPAR1uUA\n\n* 论文标题：Deep Video Discovery : Agentic Search with Tool Use for Long-form Video Understanding\n* 论文链接：https://arxiv.org/pdf/2505.18079\n\n本文提出了一种新颖的智能体 Deep Video Discovery (DVD)，通过将长视频分割为更短的片段并将其视作环境，利用 LLM 先进的推理能力来思考问题并自主规划，选择具有适当参数的工具来从环境中逐步获取信息，最终回答问题。\n\n## 深度研究Deep Research\n\n#### [腾讯AI Lab开源可复现的深度研究智能体，最大限度降低外部依赖](https://mp.weixin.qq.com/s/0-OS8FtVIGtFA5zVcX_W3w)"


[[items]]
id = "llm-3"
title = "许多 LLM 比一个更实用：多智能体协作中的道德判断"
subtopic = "多智能体系统"
topics = [
    "智能体",
    "博弈论",
...
research = []
source = "微信公众号"
summary = "研究提出中期训练策略，将 Llama 改造为更适配强化学习的推理基础模型，显著缩小其与 Qwen 在 RL 扩展能力上的差距。"
paper = "https://arxiv.org/abs/2506.20512"
code = "https://github.com/GAIR-NLP/OctoThinker"
dataset = "https://huggingface.co/OctoThinker"
link = "https://mp.weixin.qq.com/s/25wERcyTi79GOBpytujEWA"
content = "研究深入分析了 Llama 与 Qwen 等基础语言模型在强化学习训练中表现差异的成因。作者提出创新的中期训练（Mid-training）范式，在正式强化学习前增强模型的推理基础与 RL 适配性。该策略成功将 Llama 改造为更适合强化学习扩展的推理基础模型，显著缩小了其与 Qwen 的性能差距。项目已开源论文、代码、模型及相关数据资源。"
purpose = "- 为基础模型在强化学习前的能力塑造提供新的训练阶段与技术路径\n- 说明 RL 效果不仅取决于强化学习算法，也受到基础模型预训练与中期训练特性的显著影响\n- 为 Llama 等模型构建高性能推理系统提供可复现的代码、模型和数据资源\n- 有助于研究不同模型家族在 RL 扩展能力上的差异及其可干预机制"
notes = "2. 2025-07-01 10:59:44 Tuesday ｜ 首创Mid-training范式破解RL奥秘，Llama终于追平Qwen！**https://mp.weixin.qq.com/s/25wERcyTi79GOBpytujEWA**\n\n深入探讨了不同基础语言模型家族（如 Llama 和 Qwen）在强化学习（RL）训练中迥异表现的背后原因，并提出创新性的中期训练（mid-training）策略，成功地将 Llama 模型改造成高度适配强化学习的推理基础模型，显著缩小了其与天生擅长 RL 扩展的 Qwen 模型之间的性能差距，为下一代 reasoning 能力 AI 系统的开发提供了关键的科学基础和技术路径。\n\n论文链接：https://arxiv.org/abs/2506.20512\n\n代码仓库：https://github.com/GAIR-NLP/OctoThinker\n\n开源模型 & 数据：https://huggingface.co/OctoThinker"


[[items]]
id = "item-28"
title = "使用开放基准测试评估语言模型的陷阱"
subtopic = "评测可靠性"
topics = [
    "模型评估",
    "数据工程",
