...
source = "AI前线"
summary = "LLM-as-a-Verifier 通过多轨迹采样、概率化自验证与高效排序，无需微调或额外奖励模型，便将 DeepSeek V4 Flash 在 Terminal-Bench 2.1 上的成功率从 78.7% 提升至 88.0%，并以约低 11 倍的成本超过 Claude Fable 5。"
...
content = "- 该框架让同一开源 LLM 并行生成多条 Agent 轨迹，并充当验证器对完整执行过程进行细粒度评分、排序和择优，无需额外训练专用奖励模型。\n- 它利用评分 Token 的完整 logit 分布及 logprob 生成连续且包含不确定性的验证分数，避免离散评分容易出现平局的问题，并可沿评分粒度、重复验证和多评价准则三个维度扩展验证计算。\n- DeepSeek V4 Flash 在 Terminal-Bench 2.1 上的单次生成成功率为 78.7%，Best-of-3 提升至 86.5%，Best-of-5 达到 88.0%，而候选池的 Oracle 上限为 96.6%。\n- Probabilistic Pivot Tournament 通过 pivot candidates 将多候选排序复杂度从 O(N²) 降至 O(Nk)，前缀缓存则减少长轨迹重复输入带来的开销。\n- 包含轨迹生成与验证的单任务成本约为 0.11 美元；该验证机制还可用于软件工程、机器人和医疗 Agent 的轨迹排序与进度追踪，并为 SAC、GRPO 提供稠密奖励。"
purpose = "- 展示测试时扩展不应只增加候选生成量，也可将更多计算预算投入验证、排序与选择环节。\n- 为廉价开源或本地模型提供“多次采样—自我验证—择优输出”的能力提升路径，模型选型应比较单位成功任务的总成本，而非只看 Token 单价。\n- 将模型评价从离散裁判分数升级为概率化反馈，可减少候选质量接近时的平局与误选。\n- 连续验证分数可用于 Agent 轨迹级评价、实时任务进度监控，以及 SAC、GRPO 等强化学习方法的稠密奖励信号。\n- Verifier 可集成到 Agent Harness 中，作为运行时轨迹筛选、质量控制和故障恢复的基础组件；在并行资源充足时，还能以更多廉价推理计算换取能力提升，而不必随候选数量线性增加端到端延迟。"
