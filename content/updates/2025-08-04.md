...
research = []
source = "arXiv"
summary = "研究者基于 Countdown 算术游戏构建动态规划基准，以可验证、NP 完全且不易被记忆污染的问题评估大语言模型的真实规划能力。"
paper = "https://arxiv.org/abs/2508.02900"
code = ""
dataset = ""
link = "https://papers.cool/arxiv/2508.02900"
content = "论文指出，旅行规划等现有基准难以形式化验证，而国际规划竞赛题目又主要针对传统自动规划器的特定弱点。作者以 Countdown 游戏为核心生成规划任务，要求模型通过算术运算将一组输入数字组合成指定目标。该问题具有直观的自然语言描述、NP 完全的计算复杂度和丰富的实例空间，可降低训练数据记忆与基准污染的影响。实验显示，相较于固定的 24 点游戏，这一动态生成基准对现有 LLM 辅助规划方法仍极具挑战性。"
purpose = "- 用于评估大语言模型和智能体在可验证任务上的长期规划与组合搜索能力。\n- 通过动态生成实例减少题目记忆和基准污染，使模型间比较更可信。\n- 可用于分析搜索、推理及工具辅助等规划方法在问题规模增长时的性能退化。\n- 提示规划评测应同时关注形式化验证、计算复杂度与实例多样性。"
...
[[items]]
id = "deepseek-kimi"
title = "谷歌首届大模型对抗赛：DeepSeek、Kimi 等模型同场博弈"
subtopic = "博弈评测"
topics = [
    "逻辑推理",
    "博弈",
...
research = []
source = "公众号"
summary = "谷歌基于 Kaggle Game Arena 举办首届大模型对抗赛，八款主流模型将在禁止外部工具的条件下通过策略游戏展开全员循环对战。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/riLbZqmmX8xLGo_fK7VjsQ"
content = "谷歌推出公开基准测试平台 Kaggle Game Arena，让大模型在国际象棋等策略游戏中正面对抗。参赛模型包括 o4-mini、o3、DeepSeek-R1、Kimi K2 Instruct、Gemini 2.5 Pro、Gemini 2.5 Flash、Claude Opus 4 和 Grok 4。比赛采用全员循环赛制，每对模型进行大量对局，以提高排名的统计可靠性。模型不得调用 Stockfish 等外部工具，游戏执行框架与环境则将开源，并提供实时赛程、动态排行榜和技术文档。"
purpose = "- 通过策略游戏考察大模型的逻辑推理、长期规划与对手建模能力。\n- 禁用外部工具有助于更直接地衡量模型自身的推理与决策水平。\n- 开源环境和全员循环赛制可提升评测的透明度、可复现性与统计可信度。\n- 可作为传统静态问答基准之外的大模型动态交互评估范式。"
notes = "1. [谷歌约战，DeepSeek、Kimi都要上，首届大模型对抗赛明天开战](https://mp.weixin.qq.com/s/riLbZqmmX8xLGo_fK7VjsQ)\n\n   1. 参与比赛的模型包括：o4-mini（OpenAI）、DeepSeek-R1（DeepSeek）、Kimi K2 Instruct（月之暗面）、o3（OpenAI）、Gemini 2.5 Pro（谷歌）、Claude Opus 4（Anthropic）、Grok 4（xAI）、Gemini 2.5 Flash（谷歌）\n   2. 主要基于 Kaggle Game Arena [介绍](https://www.kaggle.com/blog/introducing-game-arena) ，[https://www.kaggle.com/game-arena](https://www.kaggle.com/game-arena)这是谷歌推出的一个全新的、公开的基准测试平台，在这里，AI 模型可以在策略游戏（如国际象棋和其他游戏中）中展开正面交锋，一决高下。\n\n      1. 为了确保透明性，游戏执行框架以及游戏环境本身都将开源。\n      2. 最终排名将采用严格的全员对抗赛制（all-play-all）确定，每对模型进行大量对战来确保统计结果的可靠性。\n      3. Game Arena 平台上的每款游戏均设有详情页，用户可查看：\n\n      * 实时更新的比赛对阵表；\n      * 动态排行榜数据；\n      * 该游戏对应的开源环境代码及测试框架技术文档。\n   3. 模型无法使用任何外部工具。例如，它们不能调用 **Stockfish 等国际象棋引擎**来获得最优走法。"


[[items]]
id = "item-3"
title = "元代理：通过自我进化的代理元学习工具"
subtopic = "元代理"
topics = [
    "自我进化",
    "元",
...
research = []
source = "小红书"
summary = "介绍一种通过智能体自我进化实现工具元学习的元代理思路。"
paper = ""
code = ""
dataset = ""
link = "https://www.xiaohongshu.com/discovery/item/689020cd00000000230272af?source=webshare&xhsshare=pc_web&xsec_token=CBzadlrcDTMxPZL3qy16EkiiNKvR1ap5n_BADi3h3lzSY=&xsec_source=pc_share"
content = "该内容聚焦“元代理”，探讨如何让智能体在交互与实践过程中自我进化。其核心方向是通过代理级元学习提升工具学习与使用能力，使智能体能够适应新的工具和任务。原始文本仅提供了标题及小红书分享链接，未披露具体方法、实验结果或实现细节。"
purpose = "- 为构建能够持续学习新工具的自进化智能体提供研究线索\n- 启发将元学习机制引入工具选择、调用与经验迁移过程\n- 后续需结合论文或项目资料核实技术架构与实验效果"
notes = "6. 一些局限： 1. 整个发现过程耗费了大量的计算资源（超过 20,000 GPU 小时），这表明这类研究需要强大的计算支持。 2. 尽管成果显著，但目前主要集中在线性注意力架构领域 3. 文章提到，他们目前没有投入大量工作去为新发现的架构编写定制的加速内核（如使用 Triton）。\n\n#### 10. 元代理：通过自我进化的代理元学习工具\n\n【元代理：通过自我进化的代理元学习工具 - 搬砖小牛马 | 小红书 - 你的生活兴趣社区】 😆 O6Y2LYjbq9n8lJB 😆 https://www.xiaohongshu.com/discovery/item/689020cd00000000230272af?source=webshare&xhsshare=pc_web&xsec_token=CBzadlrcDTMxPZL3qy16EkiiNKvR1ap5n_BADi3h3lzSY=&xsec_source=pc_share"
...
## [Cognitive Kernel-Pro：深度研究代理和代理基础模型训练的框架](https://papers.cool/arxiv/2508.00414)
...
## [MetaAgent：通过工具元学习走向自我进化的代理](https://papers.cool/arxiv/2508.00271)

在这项工作中，我们提出了 MetaAgent，这是一种受**边做边学**原则启发的代理范式，其中专业知识是通过实践和持续的自我完善来发展的。MetaAgent 从最小的工作流程开始，仅配备基本推理和自适应寻求帮助的能力。当遇到知识差距时，MetaAgent 会生成自然语言帮助请求，这些请求由专用工具路由器路由到最合适的外部工具。当 MetaAgent 解决任务时，它会不断进行**自我反思和答案验证**，将可作的经验提炼成简洁的文本，并动态地融入到未来的任务环境中。此外，MetaAgent 通过组织其工具使用历史记录，自主构建内部工具和持久的知识库，进一步增强其检索和整合相关信息的能力我们将这种持续的、数据驱动的过程称为 \textit{meta tool learning}，通过该过程，MetaAgent 可以逐步完善其推理和工具使用策略，而无需更改模型参数或需要进一步的后训练。
...
## [像人类一样学习：通过认知发展阶段进行资源高效的联合微调](https://papers.cool/arxiv/2508.00041)

联合微调使大型语言模型 （LLM） 能够适应下游任务，同时保护数据隐私，但其资源密集型特性限制了在边缘设备上的部署。在本文中，我们介绍了开发联合调优 （DevFT），这是一种受**认知发展启发的资源高效方法**，可在紧凑的基础上逐步构建强大的 LLM。DevFT将
