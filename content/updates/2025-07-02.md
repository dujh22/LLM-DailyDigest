...
[[items]]
id = "peva"
title = "PEVA：基于全身动作条件的第一视角视频预测世界模型"
subtopic = "世界模型"
topics = [
    "具身智能",
    "多模态",
...
research = []
source = "公众号"
summary = "伯克利与 Meta 提出的 PEVA 将完整 3D 人体姿态和历史视频帧联合建模，以预测全身动作将如何改变智能体未来的第一视角视觉观测。"
paper = "https://arxiv.org/pdf/2506.21552"
code = "https://dannytran123.github.io/PEVA/"
dataset = ""
link = "https://mp.weixin.qq.com/s/id_ISbf7wVvk3pl2GCIgWA"
content = "论文提出全身动作条件第一视角视频预测模型 PEVA，用于学习具身智能体的动作与视觉变化之间的关系。不同于仅以速度和方向为条件的传统方法，PEVA 输入包括关节位置与旋转在内的完整 3D 人体姿态，并结合历史视频帧预测未来观测。该方法使模型能够理解身体动作如何重新组织第一视角中的可见世界，为具身世界模型提供更细粒度的动作表征。"
purpose = "- 为具身智能体提供基于完整身体姿态的未来视觉预测能力\n- 支持机器人或虚拟智能体在执行动作前预演其视觉后果\n- 启示世界模型从粗粒度运动控制转向关节级、全身动作条件建模"
notes = "2025-07-02 13:11:27 Wednesday ｜ 伯克利&Meta面向具身智能的世界模型：让AI通过全身动作「看见」未来 https://mp.weixin.qq.com/s/id_ISbf7wVvk3pl2GCIgWA\n\n论文标题：Whole-Body Conditioned Egocentric Video Prediction\n\n论文地址：https://arxiv.org/pdf/2506.21552\n\n项目地址：https://dannytran123.github.io/PEVA/\n\n参考阅读链接：https://x.com/YutongBAI1002/status/1938442251866411281\n\n相比传统模型只用「速度 + 方向」做预测，PEVA 把整个人的 3D 姿态（包括关节位置和旋转）一并喂进模型，和历史的视频帧一起输入，从而让 AI 学会：身体的动作，会如何重新组织我们能看到的世界。"


[[items]]
id = "dipllm"
title = "DipLLM：以少量数据微调大模型的外交博弈智能体框架"
subtopic = "博弈智能体"
topics = [
    "博弈论",
    "智能体",
...
research = []
source = "微信公众号"
summary = "中科院团队开源DipLLM，通过自回归任务分解和均衡策略目标微调大语言模型，仅使用Cicero约1.5%的训练数据便在复杂策略游戏Diplomacy中实现更优表现。"
paper = "https://arxiv.org/pdf/2506.09655"
code = "https://github.com/KaiXIIM/dipllm"
dataset = ""
link = "https://mp.weixin.qq.com/s/Hg7vHB_2ujfKSyvAcNjn6g"
content = "DipLLM是面向七人策略游戏Diplomacy的大语言模型博弈智能体框架，该游戏同时包含合作与竞争，单轮动作空间可达10^64。框架通过自回归分解，将复杂联合决策转化为一系列可学习的序列化子任务，并使用具有理论支持的均衡策略目标进行高效微调。实验显示，DipLLM仅使用Meta Cicero约1.5%的训练数据即可取得超越后者的表现，体现出较高的样本效率。相较依赖大规模均衡搜索和重资源训练的方案，该方法为博弈智能体的扩展与迁移提供了更轻量的路径。"
purpose = "- 为复杂多智能体博弈提供低数据成本的LLM微调方案。\n- 展示自回归任务分解在超大动作空间决策中的应用价值。\n- 为减少均衡搜索与算力依赖、提升博弈智能体的迁移能力提供新思路。\n- 可用于研究协作与竞争并存场景下的策略学习和多智能体决策。"
notes = "2025-07-02 15:08:59 Wednesday｜ 策略改写「一战历史」！中科院开源全新博弈智能体框架DipLLM https://mp.weixin.qq.com/s/Hg7vHB_2ujfKSyvAcNjn6g\n\n首个在复杂策略游戏Diplomacy中基于大语言模型微调的智能体框架，仅用Cicero 1.5%的训练数据就实现超越，展现出卓越的策略能力和样本效率。该框架通过自回归分解将复杂决策任务转化为序列化子任务，结合理论支持的均衡策略目标对LLM 进行高效微调，为构建更通用、高效的博弈智能体提供了新范式。\n\n围棋、德州扑克曾是AI崛起的试炼场，从AlphaGo到Libratus，人工智能不断刷新策略上限。\n\nDiplomacy：一款融合协作与竞争的七人博弈游戏，单轮动作空间高达10的64次方，其策略建模复杂度前所未有！\n\n为此，Meta曾推出智能体Cicero[Meta, Science 2022]，结合人类数据与策略搜索，在该领域实现突破，但其方法高度依赖超大规模均衡搜索与重资源训练，难以扩展与迁移。\n\n论文地址：https://arxiv.org/pdf/2506.09655\n\n开源代码：https://github.com/KaiXIIM/dipllm\n\n#### 多智能体强化学习研究：通过自我对弈让AI发现新的技能，无需专门为这些技能设计奖励。\n\nhttps://openai.com/index/competitive-self-play/"


[[items]]
id = "dots-vlm1-sota"
title = "小红书开源首个多模态大模型 dots.vlm1，性能直追 SOTA"
subtopic = "模型发布"
topics = [
    "多模态",
    "模型开发",
...
research = []
source = "微信公众号"
summary = "小红书开源首个多模态大模型 dots.vlm1，公开信息称其综合性能已接近业界 SOTA 水平。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/aftyEJCZleUGAp0s5NBk3w"
content = "小红书发布并开源了首个多模态大模型 **dots.vlm1**。该模型面向视觉与语言信息的联合理解，公开信息称其性能已接近当前 SOTA 水平。此举体现了小红书在多模态基础模型及开源生态上的进一步布局。原始摘要未提供具体评测数据、模型规模与代码仓库地址，详细技术信息需参阅原文。"
purpose = "- 可用于跟踪国产开源多模态模型的最新进展。\n- 为视觉语言理解、内容分析等应用提供新的模型选项。\n- 可进一步关注其评测基准、训练数据、部署成本及开源许可。"
notes = "2025-07-02 13:03:30 Wednesday ｜ UofT、UBC、MIT和复旦等联合发布：扩散模型驱动的异常检测与生成全面综述 https://mp.weixin.qq.com/s/smDyNhlGS-SpM2Wt34J-ng\n\n## 模型\n\n#### [小红书开源首个多模态大模型dots.vlm1，性能直追SOTA！](https://mp.weixin.qq.com/s/aftyEJCZleUGAp0s5NBk3w)"


[[items]]
id = "ai4research"
title = "AI4Research：人工智能赋能科学研究综述"
subtopic = "科研应用"
topics = [
    "人工智能应用",
    "人工智能",
...
research = []
source = "微信公众号"
summary = "Chai-2展示了零样本设计新抗体的能力，预示AI药物设计正加速迈向临床试验阶段。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/yOd6_Qn6mlVqfRDkdS_eyw"
content = "Chai-2的发布引发医药行业关注，其核心亮点是利用AI进行零样本新抗体发现。该进展表明，蛋白质与抗体设计正在进入类似大语言模型基座化的发展阶段。相关报道预计，AI设计药物有望在2025年年底进入临床试验。延伸阅读：[蛋白质基座的GPT时代来了？！](https://mp.weixin.qq.com/s/WmGTYPO9gTm2XJ-Rjqvm-Q)。"
purpose = "- 展示AI在抗体发现与药物设计中的实际应用潜力\n- 降低候选抗体筛选对既有样本和传统实验流程的依赖\n- 关注AI生成药物从计算设计走向临床验证的关键进展"
notes = "2. 2025-07-02 15:21:56 Wednesday ｜ 诺奖得主Hassabis预言成真！AI零样本发现新抗体，轰动整个医药圈 https://mp.weixin.qq.com/s/yOd6_Qn6mlVqfRDkdS_eyw\n\nChai-2的重磅发布，意味着：在今年年底，AI设计药物有望进入临床试验。\n\n#### [蛋白质基座的GPT时代来了？！](https://mp.weixin.qq.com/s/WmGTYPO9gTm2XJ-Rjqvm-Q)"


[[items]]
id = "skywork-reward-v2"
title = "Skywork-Reward-V2：通过人机协同扩展高质量偏好数据管理"
subtopic = "奖励建模"
topics = [
    "强化学习",
    "模型训练",
...
research = []
source = "微信公众号"
summary = "浙江大学APRIL实验室联合多所高校推出UltraVideo，以高质量4K/8K视频和丰富结构化字幕支持超高清视频生成研究。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/wVLTTmbvTToW70Qqw7p80g"
content = "浙江大学APRIL实验室联合多所高校推出开源文本到视频数据集UltraVideo，旨在突破AI视频生成的清晰度瓶颈。数据集以原生UHD-4K视频为主，其中22.4%的内容达到8K分辨率，并覆盖超过100种主题。每个视频配有9条结构化字幕和1条总结性字幕，字幕平均长度为824词，可为细粒度视频语义建模提供丰富监督。"
purpose = "- 为原生4K及更高分辨率的文本到视频模型提供高质量训练数据\n- 利用多层次结构化字幕增强视频内容理解与文本对齐\n- 推动超高清视频生成模型的训练、评估和清晰度提升"
notes = "1. 2025-07-02 14:22:34 Wednesday｜ 1080p飞升4k，浙大开源原生超高清视频生成方案，突破AI视频生成清晰度上限 https://mp.weixin.qq.com/s/wVLTTmbvTToW70Qqw7p80g\n\n浙江大学APRIL实验室联合多家高校推出的 **高质量开源UHD-4K（其中22.4%为8K）文本到视频数据集——UltraVideo** ，破解了这一困局。\n\n该数据集涵盖广泛主题（超过100种），每个视频配备9个结构化字幕及一个总结性字幕（平均824词）。"


[[items]]
id = "superclue-ai"
title = "SuperCLUE推理榜惊现黑马：中兴AI模型表现亮眼"
subtopic = "模型评测"
topics = [
    "模型开发",
    "推理",
...
research = []
source = "微信公众号"
summary = "中兴旗下AI模型在SuperCLUE推理榜中取得亮眼成绩，成为榜单黑马并引发行业关注。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/H2urbOlVVcFR5b-GA7Rnhw"
content = "中兴旗下AI模型在SuperCLUE推理榜中展现出较强竞争力，成为此次榜单中的黑马。该结果反映出中兴正在加大人工智能模型研发投入，并已在推理能力评测方面取得阶段性成果。榜单表现也显示，传统通信设备厂商正进一步拓展其在大模型领域的技术布局。"
purpose = "- 关注中兴在大模型研发和推理能力优化方面的进展\n- 通过SuperCLUE榜单横向比较不同模型的综合推理表现\n- 观察传统通信厂商向AI基础模型领域延伸的行业趋势"
notes = "4. 2025-07-02 14:09:31 Wednesday ｜SuperCLUE推理榜惊现黑马：原来中兴是一家AI公司？ https://mp.weixin.qq.com/s/H2urbOlVVcFR5b-GA7Rnhw"


[[items]]
id = "cot-thinksound"
title = "阿里通义开源首个 CoT 泛音频生成模型 ThinkSound"
subtopic = "音频生成"
topics = [
    "模型开发",
    "多模态",
...
research = []
source = "微信公众号"
summary = "阿里通义语音团队开源 ThinkSound，通过思维链推理为视频画面逐帧生成匹配音效，实现更精准的音画同步。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/NPb2iQvAiTJb0LZG8CSdXg"
content = "阿里通义语音团队开源泛音频生成模型 **ThinkSound**，主要用于根据视频内容自动生成配套音效。该模型引入 CoT 推理机制，力求让每一帧画面获得语义与节奏匹配的声音，提升音画同步效果。此前，团队还发布了支持多语言语音生成和零样本语音复刻的 **CosyVoice 3.0**，以及端到端音频多模态模型 **MinMo**。这些模型共同展现了通义在语音、音效生成及音频多模态方向上的持续布局。"
purpose = "- 可用于影视、短视频、游戏等内容的自动配音与音效生成。\n- CoT 推理有望增强模型对画面事件、时序关系及声音需求的理解。\n- 展示了音频生成模型从语音合成向泛音频、多模态内容创作扩展的趋势。"
notes = "2025-07-02 14:54:01 Wednesday ｜ 阿里通义开源首个CoT音频模型，音·画同步被狠狠拿捏了 https://mp.weixin.qq.com/s/NPb2iQvAiTJb0LZG8CSdXg\n\n没错，这就是阿里通义语音团队最新开源的 **泛音频生成模型ThinkSound** ，主要用于视频配音，主打 **让每一帧画面都有专属匹配音效** 。\n\n就在上个月，团队发布了语音生成大模型 **Cosyvoice 3.0** ，通过大规模数据预训练和特殊设计的强化学习后训练，它能提供多语言语音生成、零样本语音复刻等功能。\n\n更早之前，团队还推出了基于模态对齐实现的端到端音频多模态大模型 **MinMo** 。\n\n### 智能体模型"


[[items]]
id = "claude-code-hooks-ai"
title = "Claude Code 推出 Hooks 功能，让 AI 编程走向可控工程化"
subtopic = "AI辅助编程"
topics = [
    "人工智能",
    "人工智能辅助编程",
...
research = []
source = "微信公众号"
summary = "Claude Code 发布基于 Shell 的 Hooks 功能，可在编程会话的关键节点自动执行预设操作，提升 AI 编程流程的确定性与可控性。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/7t1sJlLnA_ardC2cK_EEfg"
content = "Claude Code 推出 Hooks 功能，通过基于 Shell 的钩子系统在编程会话的关键节点自动运行命令。开发者可以精确规定每次会话中必须执行的检查、脚本或流程，减少模型行为不稳定带来的影响。该功能使 AI 编程从依赖模型临场表现，进一步转向可编排、可复现的工程化工作流。"
purpose = "- 将代码检查、测试、格式化等操作固化为自动流程\n- 增强 AI 编程会话的确定性、可控性与可复现性\n- 为 Claude Code 接入现有软件工程和自动化工具链提供基础"
notes = "1. 2025-07-02 15:30:17 Wednesday ｜ 刚刚，Claude Code推出Hooks功能，让AI 编程从「看心情」到「真工程」\n\nhttps://mp.weixin.qq.com/s/7t1sJlLnA_ardC2cK_EEfg\n\n这个基于Shell的钩子系统能在关键时刻自动运行，让你能够精确地编排每次编程会话中必须发生的事情。"


[[items]]
id = "ml-master-openai-mle-bench"
title = "上交大推出机器学习智能体 ML-Master，登顶 OpenAI MLE-bench"
subtopic = "自进化"
topics = [
    "智能体",
    "自进化",
...
research = []
source = "公众号"
summary = "DDO 在最大似然训练中融入反向 KL 散度的优化思想，无需额外判别器即可提升扩散与自回归模型的生成保真度，并刷新图像生成 SOTA。"
paper = ""
code = ""
dataset = ""
link = "https://mp.weixin.qq.com/s/tiytMxR8QJN2fFSry1mQ1w"
content = "最大似然估计（MLE）倾向于覆盖数据分布，但在模型容量有限时可能将概率质量分配到错误区域，影响生成质量。DDO 借鉴 GAN 式判别思想，在训练目标中引入反向 KL 散度成分，强化真实数据附近的密度并抑制错误区域。该方法促使模型从强调分布覆盖转向强调密度集中，从而改善生成保真度。相比直接采用 GAN 损失，DDO 避免了额外判别器及其带来的工程优化复杂性，更适合需要迭代生成的扩散和自回归模型。"
purpose = "- 为扩散模型和自回归模型提供区别于纯 MLE 的训练新范式。\n- 在有限模型容量下平衡分布覆盖与样本保真度。\n- 降低引入 GAN 式目标时对额外判别器及复杂联合优化的依赖。\n- 可用于图像生成模型训练及特定领域微调。"
notes = "2025-07-02 14:10:32 Wednesday ｜ ICML 2025 Spotlight | 清华朱军组&NVIDIA提出DDO：扩散/自回归模型训练新范式，刷新图像生成SOTAhttps://mp.weixin.qq.com/s/tiytMxR8QJN2fFSry1mQ1w\n\n为解决 MLE 的局限性，文章考虑使用 **GAN 式判别**的思想，在训练目标中引入**反向 KL 散度**的成分，强化模型在真实数据附近的密度，同时抑制错误区域，将模型分布由图（a）：强调密度覆盖，微调为图（b）：强调密度集中，从而提高生成保真度与有限模型容量下的生成质量。然而， **直接使用 GAN 损失会引入额外的判别器网络与工程优化上的复杂性** ，尤其对于扩散/自回归模型这类需要迭代式多步生成的模型。\n\n### 领域微调"


[[items]]
id = "sciarena"
title = "SciArena：面向科学文献任务的开放式基础模型评估平台"
subtopic = "科研评测"
topics = [
    "模型评估",
    "智能体工具",
