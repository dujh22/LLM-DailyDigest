...
- **判重 key 扩展**：除四个 URL 字段外，`notes` 原始笔记中出现的 arXiv / GitHub 链接也提取为判重 key（信源常把论文链接留在正文而未填入 `paper` 字段），让「同一工作、不同信源」的条目共享硬证据。

### 当日整备（语义去重 + 每日摘要）`POST /api/finalize`

URL 判重只能覆盖共享链接的重复；同一工作被两个信源以不同表述收录（无共同链接）时，由**当日整备**兜底。整备做三件事（幂等，可重复执行，多次提交后结果收敛）：

1. **当日 URL 去重**：对目标日期做一次上述 URL 判重归并（含 notes 提取 key）；
2. **LLM 语义去重（自动应用）**：当天全部条目（id/标题/摘要/来源/链接）一次交给 LLM 找出「同一工作/事件」的分组，prompt 约束**宁可漏合、不可错合**，并有代码级护栏——组内条目 `paper` 指向不同 arXiv 论文时整组拒绝；分组后复用 `/dedup` 的吸收合并 + LLM 解析字段整合管线，锁内按 id 重新定位校验后写盘（`notes` 原始笔记逐字保留，错并可回溯恢复）；
3. **重生成日报头部摘要**：一次 LLM 调用生成两节 markdown——「今日概览」（按主线聚类 3~6 个要点，可含 `[标题](#条目id)` 锚点）与「对当前研究的启发」（只列当天确有启发的研究项目，每个一句话），写入日报正文 `<!-- daily-summary:start/end -->` 标记对之间，**整块替换**保证幂等。

**触发时机**（三种，均自动去重叠）：批处理「一键自动处理」全部提交完成后立即执行一次；单条提交后防抖执行（最后一次提交后静默 `FINALIZE_DEBOUNCE` 秒，默认 600）；手动 `curl -X POST http://localhost:5050/api/finalize -H 'Content-Type: application/json' -d '{"date": "2026-09-22"}'`（date 缺省为今天，同步执行需 1~2 分钟）。无 API Key 时仅做规则 URL 去重，跳过语义去重与摘要。
